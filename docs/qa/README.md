...
| 画面の数 | メニューから行ける画面 **6** ／ 案内ページ **5** ／ 記事ページ **558** ／ メニューに出ない画面 **1** |
| 作成日 | 2026年8月4日 |
...
| 01 | [ホーム・ログイン・案内ページ](./01-home.md) | `/` `/login` `/about` `/contact` `/privacy` `/terms`（`/dashboard` は削除済みの記録） |
| 02 | [物件を方位で探す](./02-arbitrage.md) | `/relocation/arbitrage`（第2部の `/rentals` は削除済みの記録） |
| 03 | [引越し先を試算する／過去の移動履歴](./03-simulator.md) | `/relocation/simulator` `/relocation/history` |
| 04 | [移住先の地域を比べる](./04-wealth.md) | `/relocation/wealth` |
| 05 | [本命星と吉方位を調べる（558ページ）](./05-houi.md) | `/houi` 配下すべて |
...
> **メモ**：直接ログイン画面からログインした場合、行き先は**トップ（`/`）**です。
> 以前は `/rentals`（削除済み）へ飛んでいて「見たことのない英語の画面が出た」という
> 報告がありましたが、`next` の既定値がトップに変わって解消しています。
...
**すべてログイン不要です。合計558ページ**（`/houi` 自体もこの558に含まれます）。
...
| `/houi/area/{コード}` | 〇〇市から見た方位別のエリア | 313 | [05](./05-houi.md#8-houiareaコード-エリア詳細) |
...
### 3-4. メニューに出ない画面（URL直打ちのみ・1画面／ログイン必要）
...
`/dashboard`（全機能ランチャー。ログイン後の行き先はトップに変更）
...
| 3 | ~~`/rentals` で物件の「シミュレート」を押す~~ | **確認不要。**`/rentals` は削除済みで、この引き継ぎ経路は現在ありません |
...
3. **ログインの遮断**：`/relocation/history` と `/admin` で、未ログイン時にログイン画面へ飛ぶこと
   （保護対象はこの 2 つだけです。3-4 を参照）
4. 404の確認
...
12. （[07](./07-other.md) は削除済みの画面の記録です。**確認するものはありません**）
...
| 症状 | 仕様どおりの可能性 | 詳細 |
|---|---|---|
| 物件が少ししか出ない | **築年数上限の初期値が「5」**になっている | [02](./02-arbitrage.md#5-条件②絞り込みフィルター) |
...
| 東京のエリアページが無い | 取り込み対象に加えたばかりで、まだ物件が溜まっていない。エリアページは掲載30件以上の市区町村にだけ立つ | [05](./05-houi.md#2-4-エリアコード) |
...
| 4 | ~~ログイン後の既定の行き先が `/rentals`~~ **解消済み**。既定はトップ（`/`） | [01](./01-home.md#4-authcallback-ログイン後の受け渡し画面なし) |
