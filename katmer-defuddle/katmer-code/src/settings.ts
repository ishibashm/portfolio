...
export class ClaudeNativeSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Default working directory for Claude sessions (empty = vault root)",
      )
      .addText((text) =>
        text
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "How Claude handles tool approvals. 'Accept edits' auto-approves file changes only.",
      )
      .addDropdown((dropdown) =>
        dropdown
          .addOptions({
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Auto-approve WebFetch, WebSearch, curl, python3, and open commands. Required for academic skills (/lit-search, /cite-verify, etc.)",
      )
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.allowWebRequests)
          .onChange((value) => {
      ...
      .addButton((btn) =>
        btn.setButtonText("Enable all").onClick(() => {
          ...
          if (!this.plugin.settings.allowWebRequests) {
            ...
            new Notice(
              `${SKILL_CATALOG.length} skills enabled + web requests allowed`,
            );
          } else {
          ...
          .addToggle((toggle) =>
            toggle.setValue(enabled).onChange((value) => {
              if (value) {
                ...
                if (
                  !this.plugin.settings.allowWebRequests &&
                  skill.id !== "abstract" &&
                  skill.id !== "report-template"
                ) {
                  new Notice(
                    'This skill uses web APIs (CrossRef, Semantic Scholar, etc.). Enable "Allow web requests" above for it to work.',
                    8000,
                  );
                }
              } else {
                this.plugin.settings.enabledSkills =
                  this.plugin.settings.enabledSkills.filter(
                    (id) => id !== skill.id,
                  );
              }
              void this.plugin.saveSettings();
