...
).require("electron").shell as { openPath: (path: string) => Promise<string> };
...
export default class ClaudeNativePlugin extends Plugin {
  ...
  private pickAndOpenReport(): void {
    ...
    try {
      ...
      const modal = new ReportPickerModal(
        this.app,
        files,
        reportsDir,
        (fp) => void this.openReport(fp),
      );
      modal.open();
    } catch {
      ...
      this._reportWatcher = watch(
        ...
        (_eventType: string, filename: string) => {
          ...
          setTimeout(() => {
            ...
            el.createEl("div", {
              cls: "katmer-report-notice-title",
              text: "Report ready",
            });
            el.createEl("div", {
              cls: "katmer-report-notice-file",
              text: label,
            });
          ...
          if (existsSync(targetPath)) {
            try {
              ...
              if (!existing.startsWith("<!-- KatmerCode skill:")) {
                ...
                new Notice(
                  `Skill "${skill.name}" not installed: ~/.claude/commands/${skill.fileName} already exists (not owned by KatmerCode).`,
                );
                canWrite = false;
              }
            } catch {
  ...
  private async saveSession(session: SavedSession): Promise<void> {
    const idx = this.settings.sessions.findIndex(
...
class ReportPickerModal extends FuzzySuggestModal<string> {
  ...
  getItemText(file: string) {
    return file.replace(".html", "").replace(/-/g, " ");
  }
  onChooseItem(file: string) {
    this.onChooseFn(join(this.dir, file));
  }
}
