...
export class ProcessManager {
  ...
  private createCustomSpawn(): (options: SpawnOptions) => SpawnedProcess {
    return (options: SpawnOptions): SpawnedProcess => {
      ...
      if (command === "node") {
        ...
        if (
          spawnArgs.length > 0 &&
          !spawnArgs[0].endsWith(".js") &&
          !spawnArgs[0].endsWith(".mjs")
        ) {
          command = spawnArgs[0];
  ...
  private buildOptions(): Options {
    ...
    const options: Options = {
      ...
      canUseTool: this.onPermissionRequest
        ? async (toolName, input, opts) => {
            ...
            try {
              const result = await new Promise<"allow" | "deny" | "always">(
                (resolve, reject) => {
                  if (signal?.aborted) {
              ...
              if (result === "allow" || result === "always") {
                return {
                  ...
                  updatedInput: input,
                  updatedPermissions:
                    result === "always" ? opts.suggestions : undefined,
                };
              }
    ...
    try {
      ...
      if (!this._aborted) {
        ...
        if (
          ...
          this.onStderr?.(
            "Not logged in. Please run `claude` in your terminal first to authenticate.",
          );
        }
        this.setState("error");
        this.onComplete?.();
  ...
  private handleSDKMessage(msg: SDKMessage): void {
    switch (msg.type) {
      case "system": {
        ...
        if ("subtype" in sysMsg && sysMsg.subtype === "compact_boundary") {
          this.onEvent?.({
            ...
            ...(sysMsg as SDKCompactBoundaryMessage).compact_metadata,
          });
        }
        break;
      }
      ...
      case "assistant": {
        ...
        const event: AssistantMessageEvent = {
          ...
          message: {
            ...
            content:
              (aMsg.message
                ?.content as AssistantMessageEvent["message"]["content"]) || [],
            ...
            usage: {
              ...
              cache_read_input_tokens:
                aMsg.message?.usage?.cache_read_input_tokens || 0,
              cache_creation_input_tokens:
                aMsg.message?.usage?.cache_creation_input_tokens || 0,
            },
          },
      ...
      case "result": {
        ...
        const event: ResultEvent = {
          ...
          usage: {
            ...
            cache_creation_input_tokens:
              rMsg.usage?.cache_creation_input_tokens || 0,
          },
        };

        // Attach modelUsage for context window extraction
        if (rMsg.modelUsage) {
          (event as unknown as Record<string, unknown>).modelUsage =
            rMsg.modelUsage;
        }
      ...
      case "stream_event": {
        ...
        const streamMsg = msg as unknown as {
          type: "stream_event";
