...
const DiffMatchPatch =
  (DiffMatchPatchModule as unknown as { default: typeof DiffMatchPatchModule })
    .default || DiffMatchPatchModule;
...
class ChangeHoverWidget extends WidgetType {
  constructor(private changeId: string) {
    super();
  }
  eq(other: ChangeHoverWidget) {
...
const changeDecorations = StateField.define<DecorationSet>({
  ...
  update(_decos, tr) {
    ...
    decos.sort(
      (a, b) => a.from - b.from || a.to - b.to || a.sortSide - b.sortSide,
    );
...
export function goToPreviousChange(view: EditorView): boolean {
  ...
  const cursor = view.state.selection.main.head;
  const prev =
    [...pending].reverse().find((c) => c.from < cursor) ||
