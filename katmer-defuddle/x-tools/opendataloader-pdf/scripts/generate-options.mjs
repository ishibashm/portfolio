...
function generateNodeConvertOptions() {
    ...
    for (const opt of options.options) {
        ...
        if (opt.type === "boolean") {
            ...
            lines.push(`  if (cliOptions.${camelName}) {`);
            lines.push(
                `    convertOptions.${camelName} = cliOptions.${camelName};`,
            );
            lines.push("  }");
        }
    }
    ...
    for (const opt of options.options) {
        ...
        if (opt.type === "boolean") {
            ...
            lines.push(
                `        args.push('${cliFlag}', options.${camelName}.join(','));`,
            );
            lines.push("      }");
            lines.push("    } else {");
...
function generatePythonCliOptions() {
    ...
    for (const opt of options.options) {
        ...
        lines.push(
            `        "short_name": ${opt.shortName ? `"${opt.shortName}"` : "None"},`,
        );
        lines.push(`        "type": "${opt.type}",`);
        lines.push(`        "required": ${opt.required ? "True" : "False"},`);
        lines.push(`        "default": ${defaultValue},`);
        lines.push(
            `        "description": "${escapeString(opt.description, '"', { escapePercent: true })}",`,
        );
        lines.push("    },");
    }
...
function generatePythonConvert() {
    ...
    for (const opt of options.options) {
        ...
        if (opt.type === "boolean") {
            ...
            lines.push(`            if ${snakeName}:`);
            lines.push(
                `                args.extend(["${cliFlag}", ",".join(${snakeName})])`,
            );
            lines.push(`        else:`);
...
function generatePythonConvertOptionsMdx() {
    ...
    rows.push([
        "`input_path`",
        String.raw`\`str \| list[str]\``,
        "required",
        "One or more input PDF file paths or directories",
    ]);
...
function generateNodeConvertOptionsMdx() {
    ...
    lines.push(
        ...formatTable(["Option", "Type", "Default", "Description"], rows),
    );
    lines.push("");
