...
export async function sendWebhookNotification(properties: any[], config: WebhookConfig) {
    ...
    if (isDiscord) {
      ...
      for (let i = 0; i < filtered.length; i += batchSize) {
        ...
        const embeds = batch.map(p => {
          ...
          else if (p.building_age !== null && p.building_age <= 5) titleTags.push("🆕築浅");
          ...
          return {
            title: title,
            url: dashboardLink,
            color: p.building_age !== null && p.building_age <= 5 ? 16760100 : 1096065, // Gold (FBBF24) or Emerald (10B981) in decimal
            fields: [
              {
                name: "賃料 (管理費)",
...
export async function sendTestNotification(url: string) {
  if (!url) return { success: false, error: "Webhook URLが入力されていません。" };
  ...
  try {
    ...
    return { success: false, error: e.message || "Webhookの送信に失敗しました。" };
  }
}
  ...
  try {
    ...
    if (data && typeof data === 'object') {
      if ('success' in data) {
        ...
        if (isSuccess && webhookConfig && webhookConfig.enabled && webhookConfig.url) {
