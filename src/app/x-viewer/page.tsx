  ...
  const displayedPosts = useMemo(() => {
    ...
    if (viewMode === "gallery") {
      result = result.filter(
        (p) => Array.isArray(p.saved_media) && p.saved_media.length > 0,
      );
    }
              ...
              <li>
                ...
                >
                  ...
                  <div className="truncate">
                    <p
                      className={`font-semibold text-sm truncate ${selectedAccount === "ALL" ? "text-blue-700 dark:text-blue-300" : "text-gray-900 dark:text-gray-100"}`}
                    >
                      すべてのアカウント
                    </p>
              ...
              {filteredAccounts.map((account) => (
                <li key={account}>
                  ...
                  >
                    ...
                    <div className="truncate">
                      <p
                        className={`font-semibold text-sm truncate ${selectedAccount === account ? "text-blue-700 dark:text-blue-300" : "text-gray-900 dark:text-gray-100"}`}
                      >
                        {account}
                      </p>
                      <p className="text-xs text-gray-500 truncate">
              ...
              {loadingPosts ? (
                ...
                <div className="p-4 grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {displayedPosts.map((post, index) => {
                    ...
                    return (
                      <div
                        key={(post.id as string) || index}
                        className="group relative aspect-square rounded-xl overflow-hidden bg-gray-200 dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-800"
                      >
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-3">
                          <p className="text-white text-xs line-clamp-2 text-shadow-sm">
                            {post.text as string}
                          </p>
                        </div>
                      </div>
