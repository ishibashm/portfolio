...
export async function GET(request: Request) {
  ...
  */
  ...
  try {
    ...
    //
    // work_mem をここで広げてはいけない。一度 $transaction(async tx =>
    // SET LOCAL ...) で包んだところ、Prisma のインタラクティブ
    // トランザクションには**既定 5 秒のタイムアウト**があり、22 秒かかる
    // 全国走査がトランザクションごと閉じられて 500 になった（本番で実際に
    // 落とした）。しかも EXPLAIN の実測で work_mem 512MB はディスク溢れを
