...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!apiKey) {
      ...
      console.warn(
        "EDINET_API_KEY is not set. Returning empty list or mock data.",
      );
      return NextResponse.json(
        {
          error:
            "EDINET APIキーが設定されていません。.envに EDINET_API_KEY を設定してください。",
        },
        { status: 500 },
      );
    }
    ...
    if (!response.ok) {
      throw new Error(
        `EDINET API responded with ${response.status}: ${response.statusText}`,
      );
    }
