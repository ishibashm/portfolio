...
function getValueFromCandidates(dataMap: Record<string, number>, candidates: string[]): number | null {
...
export async function GET(
  ...
  try {
    ...
    if (!company) {
      return NextResponse.json({ error: 'Company not found in DB' }, { status: 404 });
    }
    ...
    let operating_income = getValueFromCandidates(dataMap, SUMMARY_ITEMS.OperationIncome);
    let ordinary_income = getValueFromCandidates(dataMap, SUMMARY_ITEMS.OrdinaryIncome);
