...
export async function GET() {
  ...
  // Safe parsing to avoid showing password
  ...
  if (dbUrl) {
    try {
      ...
      parsedDbUrl = `Error parsing URL: ${e.message}. Length: ${dbUrl.length}`;
    }
  }
  ...
  if (directUrl) {
    try {
      ...
      parsedDirectUrl = `Error parsing URL: ${e.message}. Length: ${directUrl.length}`;
    }
  }
