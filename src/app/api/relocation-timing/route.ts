...
export async function GET(req: Request) {
  ...
  const centerStarNum = targetYear === 2026 ? 1 : (2026 - targetYear + 1) % 9 || 9;
...
出力はJSONフォーマット（bestDirection, score, timing, reason）のみとし、ダッシュボードのUIに直接表示するため、緊迫感のある「作戦指令風」のトーンで出力してください。`,
      ...
      tools: {
        get_kyusei_board: tool({
          description: '指定された年の中宮星の番号から、その年の九星気学の盤面（9方位の星の配置）を取得する',
          // @ts-ignore
          parameters: z.object({
            centerStarNumber: z.number().describe('その年の中宮の星番号（例：2026年なら1）'),
          }),
        }),
      },
    });
    ...
    if (response.toolCalls && response.toolCalls.length > 0) {
      ...
      if (call.toolName === 'get_kyusei_board') {
        ...
        const directions = ['South-East', 'South', 'South-West', 'East', 'Center', 'West', 'North-East', 'North', 'North-West'];
