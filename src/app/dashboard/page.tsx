...
import { Building, TrendingUp, Compass, ArrowUpRight, Activity, MapPin, JapaneseYen, Clock, Sparkles, BookOpen, BrainCircuit, HeartPulse, Globe, Zap, Shield, BatteryCharging, Radio, Moon, Palette } from "lucide-react"
...
export default async function DashboardPage() {
  ...
  const [realEstates, stocks, timings, ouraReadiness, tavilyResult, wealthData, latestVisualization] = await Promise.all([
    ...
    prisma.visualizedComponent.findFirst({
      orderBy: { createdAt: 'desc' }
    }).catch((e: any) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 auto-rows-[minmax(180px,auto)] gap-6">
          ...
          <section className="lg:col-span-2 lg:row-span-2 p-6 rounded-3xl bg-white/[0.02] border border-white/10 backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] hover:bg-white/[0.03] transition-all flex flex-col">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-2xl font-semibold tracking-tight text-white/90">クオンツ株式シグナル</h2>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-grow">
              {stocks.length === 0 ? (
                <div className="col-span-2 p-6 rounded-2xl bg-white/5 border border-white/5 text-center flex items-center justify-center">
                  <p className="text-gray-500 text-sm">株式シグナルはまだ生成されていません。</p>
                </div>
              ) : (
          ...
          <section className="lg:col-span-1 lg:row-span-1 p-6 rounded-3xl bg-gradient-to-br from-indigo-500/10 to-transparent border border-indigo-500/20 backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 mb-4 relative z-10">
              ...
              <h2 className="text-lg font-semibold tracking-tight text-indigo-50">アクションエンジン</h2>
            </div>
              ...
              {nbaResult ? (
                <>
                  <div className="flex items-end justify-between">
                    <div>
                      <p className="text-xs text-indigo-300/70 uppercase tracking-widest mb-1">推奨アクション (Recommended)</p>
                      <h3 className="text-2xl font-bold text-white tracking-tight">{nbaResult.suggestedAction}</h3>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-indigo-300/70 uppercase tracking-widest mb-1">確信度 (Confidence)</p>
                ...
                <p className="text-sm text-gray-500">RLエージェント オフライン</p>
              )}
            </div>
          </section>
          ...
          <section className="lg:col-span-1 lg:row-span-1 p-6 rounded-3xl bg-white/[0.02] border border-white/10 backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] hover:bg-white/[0.03] transition-all">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold tracking-tight text-white/90">生体情報 (Biometrics)</h2>
            </div>
            ...
            {readinessData ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-center gap-2 opacity-50">
                ...
                <p className="text-xs text-gray-500">Oura API未接続またはキーがありません</p>
              </div>
            )}
          </section>
          ...
          <section className="lg:col-span-2 lg:row-span-1 p-6 rounded-3xl bg-white/[0.02] border border-white/10 backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] hover:bg-white/[0.03] transition-all flex flex-col">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold tracking-tight text-white/90">マクロ環境インテリジェンス</h2>
            </div>
          ...
          <section className="lg:col-span-1 lg:row-span-1 p-6 rounded-3xl bg-white/[0.02] border border-white/10 backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] hover:bg-white/[0.03] transition-all flex flex-col">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold tracking-tight text-white/90">アストロ・タイミング</h2>
            </div>
            ...
            <div className="mb-4 bg-gradient-to-r from-purple-900/20 to-transparent p-3 rounded-xl border border-purple-500/20">
              <div className="flex justify-between items-center mb-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-purple-300 font-medium">月相・六曜 (Lunar & Rokuyo)</span>
                </div>
            ...
            <div className="flex flex-col gap-3 flex-grow">
              {timings.length === 0 ? (
                <p className="text-gray-500 text-xs text-center py-2 opacity-60">今後の吉日データはありません。</p>
              ) : (
          ...
          <section className="lg:col-span-2 lg:row-span-1 p-6 rounded-3xl bg-white/[0.02] border border-white/10 backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] hover:bg-white/[0.03] transition-all flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-semibold tracking-tight text-white/90">Wealth & Relocation Matrix</h2>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 flex-grow">
              {wealthData.length === 0 ? (
                <div className="col-span-3 p-4 rounded-xl bg-white/5 border border-white/5 text-center flex items-center justify-center">
                  <p className="text-gray-500 text-xs">所得統計データはありません。</p>
                </div>
              ) : (
          ...
          <section className="lg:col-span-2 lg:row-span-1 p-6 rounded-3xl bg-white/[0.02] border border-white/10 backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] hover:bg-white/[0.03] transition-all flex flex-col justify-between">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold tracking-tight text-white/90">AIビジュアライザ・ギャラリー</h2>
                  <p className="text-[11px] text-gray-500 font-light">最新の生成コンポーネント</p>
                </div>
              </div>
              {latestVisualization && (
                <Link 
            ...
            {latestVisualization ? (
              <div className="flex flex-col sm:flex-row gap-4 items-center flex-grow">
                ...
                <div className="w-full sm:w-1/3 flex flex-col justify-between h-full py-1">
                  <div>
                    <h3 className="font-semibold text-white text-sm line-clamp-1 mb-1">{latestVisualization.title}</h3>
                    <p className="text-xs text-indigo-300 font-mono mb-2 uppercase tracking-wider">{latestVisualization.style}</p>
                    <p className="text-[11px] text-gray-400 line-clamp-2 leading-relaxed">
                      生成日: {new Date(latestVisualization.createdAt).toLocaleDateString()}
                    </p>
                  </div>
                  <div className="mt-4">
                    <Link
              ...
              <div className="flex-grow flex flex-col items-center justify-center text-center p-6 border border-dashed border-white/10 rounded-2xl bg-white/[0.01]">
                ...
                <p className="text-sm font-medium text-gray-400">コンポーネントはまだ生成されていません</p>
