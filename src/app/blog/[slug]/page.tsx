        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-start">
          <article>
            ...
            <div className="mt-10 border-t border-slate-300 pt-6">
              <Link href="/blog" className="text-sm font-bold text-rose-600 hover:underline">
                ← 記事一覧へ戻る
              </Link>
            </div>
          </article>
          ...
          <aside className="space-y-5 lg:sticky lg:top-8">
            <section className="rounded-2xl border border-slate-300 bg-white/80 p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <span key={tag} className="rounded-full bg-stone-100 px-2.5 py-1 text-[11px] text-slate-600">
                    #{tag}
                  </span>
                ))}
              </div>
