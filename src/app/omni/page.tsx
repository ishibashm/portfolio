  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `/api/omni/stock?ticker=${encodeURIComponent(ticker)}`,
      );
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/10 pb-6">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              統合株価分析・AI予測ダッシュボード (Integration Layer)
            </p>
          </div>
              ...
              <div className="flex items-center gap-6 w-full md:w-auto">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    {stockData.summary?.name || "Unknown Company"} |{" "}
                    {stockData.summary?.sector}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
                <div className="text-right">
                  <p className="text-3xl font-mono font-bold text-emerald-400">
                    {stockData.summary?.currentPrice
                      ? stockData.summary.currentPrice.toLocaleString()
                      : "---"}
                  </p>
                  {stockData.summary?.currentPrice &&
                    stockData.summary?.previousClose && (
                      <p
                        className={`text-sm font-bold flex items-center justify-end gap-1 ${stockData.summary.currentPrice >= stockData.summary.previousClose ? "text-emerald-500" : "text-red-500"}`}
                      >
                        <TrendingUp
                          className={`w-4 h-4 ${stockData.summary.currentPrice < stockData.summary.previousClose ? "rotate-180" : ""}`}
                        />
                        {(
                          ((stockData.summary.currentPrice -
                            stockData.summary.previousClose) /
                ...
                <div className="text-right">
                  ...
                  <span className="font-mono text-gray-300">
                    {stockData.summary?.marketCap
                      ? formatNumber(stockData.summary.marketCap)
                      : "---"}
                  </span>
                </div>
              </div>
              ...
              {activeTab === "chart" && stockData.chart && (
                <div className="w-full h-[450px]">
                  <h3 className="text-sm font-bold text-gray-300 mb-4 flex items-center gap-2">
                    <Activity className="w-4 h-4" /> Price History & TimesFM
                    Forecast
                  </h3>
              ...
              {activeTab === "fundamentals" && (
                <div className="h-[500px] w-full">
                  <IRAnalysisWidget
                    ...
                    companyName={stockData.summary?.name}
                  />
                </div>
              )}
              ...
              {activeTab === "forecast" && (
                <div className="flex flex-col items-center justify-center h-[400px] text-center space-y-4">
                  ...
                  <h3 className="text-xl font-bold text-white">
                    TimesFM Price Prediction
                  </h3>
                  <p className="text-sm text-gray-400 leading-relaxed max-w-md">
                    This will wire up the <code>timesfm</code> Python worker
                    using historical data from the technical chart to project a
                    30-day forward trajectory.
                  </p>
                  <button
