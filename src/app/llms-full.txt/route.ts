...
- [鑑定士の掲載](${baseUrl}/practitioners): 引越しの方位と日取りを扱う鑑定士の一覧。
  掲載は登録制で、内容は各鑑定士によるものです。当サイトが鑑定の結果を保証するものでは
  ありません。
- [物件を掲載したいオーナーの方へ](${baseUrl}/owners): 掲載の考え方と、当サイトが
  しないこと。**媒介・代理（条件の交渉、契約書の作成、重要事項説明）は行いません。**
  宅地建物取引業の免許が要る業務で、当サイトは免許を持っていません。掲載を始めた場合も
  役割は物件を見つけてもらうところまでで、取引態様（貸主か媒介か）を明示し、募集の
  終わった物件は下げます。**掲載の受け付けは準備中です。**
