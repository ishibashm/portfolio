...
### 1. 入力パラメータの解説と現状分析
まず最初に、上記のデータセット（特に「出生時と現在の天体黄経の位相差（木星など）」、「局所地磁気」「宇宙天気」「自律神経負荷(ANS Load)」など）が、現代の地球物理学や生体アルゴリズムの視点から見て『現在のユーザーのコンディションや環境ストレス』に対してどのような意味や影響を持っているのか、パラメータの持つ物理的な意味を分かりやすく解説してください。

### 2. 専門家会議による戦略的見解
次に、対象のアクション「${actionIntent}」を実行すべきか避けるべきかについて、以下の5人の専門家のパラグラフを含めて具体的な戦術分析をまとめてください：
