...
import { BookOpen, Sparkles, Wind, Flame, Mountain, Scissors, Droplets } from "lucide-react";
...
export const BaziReport = ({ data }: { data: BaziData }) => {
  const getElementIcon = (wuxing: string) => {
    if (wuxing.includes('木')) return <Wind className="w-4 h-4 text-emerald-400" />;
    if (wuxing.includes('火')) return <Flame className="w-4 h-4 text-red-400" />;
    if (wuxing.includes('土')) return <Mountain className="w-4 h-4 text-amber-600" />;
    if (wuxing.includes('金')) return <Scissors className="w-4 h-4 text-zinc-300" />;
    if (wuxing.includes('水')) return <Droplets className="w-4 h-4 text-blue-400" />;
    return null;
  };

  const renderPillar = (title: string, pillar: BaziPillar) => (
    <div className="flex flex-col items-center p-4 bg-black/40 border border-white/5 rounded-2xl">
      <span className="text-[10px] text-gray-500 uppercase tracking-widest mb-3 font-bold">{title}</span>
      <div className="flex flex-col items-center gap-1 mb-3">
        <div className="text-3xl font-black text-white flex gap-1">
          <span className="hover:text-emerald-400 transition-colors cursor-default">{pillar.gan}</span>
          <span className="hover:text-blue-400 transition-colors cursor-default">{pillar.zhi}</span>
        </div>
      ...
      <div className="w-full space-y-2 border-t border-white/5 pt-3">
        ...
        <div className="flex justify-between items-center text-[10px]">
          ...
          <span className="text-emerald-400 font-bold">{pillar.lifeStage}</span>
        </div>
        <div className="flex justify-between items-center text-[10px]">
          <span className="text-gray-500">納音</span>
          <span className="text-purple-300 truncate ml-2">{pillar.nayin}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="p-6 rounded-[2rem] bg-gradient-to-br from-white/5 to-transparent border border-white/10">
          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-amber-400" /> 基本性格とエネルギー状態
          </h4>
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <div className="text-4xl font-black text-white bg-white/5 w-16 h-16 rounded-2xl flex items-center justify-center border border-white/10">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-white">身強/身弱: {data.summary.strength}</p>
              </div>
            </div>
        ...
        <div className="p-6 rounded-[2rem] bg-black/40 border border-white/5">
          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">五行バランス (Five Elements)</h4>
          <div className="space-y-4">
            {Object.entries(data.fiveElements).map(([element, count]) => (
              <div key={element}>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="text-gray-400 font-mono">{count} units</span>
                </div>
                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                  <motion.div 
                    ...
                    className={`h-full rounded-full ${
                      element === '木' ? 'bg-emerald-500' :
                      element === '火' ? 'bg-red-500' :
                      element === '土' ? 'bg-amber-600' :
                      element === '金' ? 'bg-zinc-300' : 'bg-blue-500'
                    }`}
                  />
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        ...
        <div className="p-6 rounded-[2rem] bg-white/5 border border-white/10">
          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">システム解析詳細</h4>
          <div className="space-y-2 text-[10px] text-gray-400">
            <div className="flex justify-between">
              <span>真太陽時補正 (TST)</span>
              <span className="text-blue-300 font-mono">Active</span>
            </div>
            <div className="flex justify-between">
              <span>国立天文台データ同期</span>
              <span className="text-blue-300 font-mono">1900-2100 Accurate</span>
            </div>
            <div className="flex justify-between">
              <span>蔵干通変星抽出</span>
              <span className="text-blue-300 font-mono">Full Trace</span>
            </div>
          </div>
        </div>
      </div>
              ...
              <div className="flex gap-4 pb-2">
                  {data.luckCycles?.map((cycle, i) => (
                      <div key={i} className="flex-shrink-0 w-24 p-3 rounded-xl bg-black/40 border border-white/5 text-center">
                          ...
                          <p className="text-[9px] text-purple-400 font-bold">10年運</p>
                      </div>
                  ))}
              </div>
          </div>
