...
export const BaziReport = ({
  ...
  const renderPillar = (title: string, pillar: BaziPillar) => (
    <div className="flex flex-col items-center p-4 bg-black/40 border border-white/5 rounded-2xl">
      <span className="text-[10px] text-gray-500 uppercase tracking-widest mb-3 font-bold">
        {title}
      </span>
            ...
            <div className="pt-4 border-t border-white/5">
              ...
              <div className="flex flex-wrap gap-2">
                {data.shenSha?.map((s) => (
                  <span
                    key={s}
                    className="px-2 py-1 rounded-md bg-black/40 border border-white/10 text-[10px] text-amber-300"
                  >
                    {s}
                  </span>
                ))}
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <div className="flex gap-4 pb-2">
            {data.luckCycles?.map((cycle, i) => (
              <div
                key={i}
                className="flex-shrink-0 w-24 p-3 rounded-xl bg-black/40 border border-white/5 text-center"
              >
