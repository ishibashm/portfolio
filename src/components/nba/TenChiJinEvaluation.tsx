...
import { Direction, StarFrequency, getClassicalYearStar, getPersonalVoidZodiac } from '@/utils/ephemerisEngine';
...
interface TenChiJinEvaluationProps {
  ...
  onApplyAction?: (actionType: 'DETOUR' | 'DATE' | 'NAVIGATE' | 'REST', data?: any) => void;
...
export function TenChiJinEvaluation({
  ...
  const mainUserStar = useMemo(() => getClassicalYearStar(mainUserBirthDateObj), [mainUserBirthDateObj]);
  ...
  const timeMetrics = useMemo(() => {
    ...
    stepsToEvaluate.forEach(step => {
      ...
      if (ev) {
        ...
        const qPercent = Math.max(0, Math.min(100, Math.round((ev.qValue + 100) / 2)));
        totalScore += qPercent;
        count++;
        ...
        Object.keys(nbaEvaluations).forEach(dateKey => {
          ...
          if (testEv && testEv.qValue > ev.qValue && testEv.qValue > bestAlternativeQ) {
  ...
  const spaceMetrics = useMemo(() => {
    ...
    stepsToEvaluate.forEach(step => {
      ...
      if (['NOISE_GOU', 'NOISE_ANKEN', 'NOISE_HA'].includes(status)) {
        hasSevereClash = true;
        worstRating = '大凶';
        worstClashType = status === 'NOISE_GOU' ? '五黄殺' : status === 'NOISE_ANKEN' ? '暗剣殺' : '歳破';
      }
  ...
  const humanMetrics = useMemo(() => {
    ...
    const personalCondition = Math.max(0, Math.min(100, Math.round(((100 - simulatedAns) + simulatedShield) / 2)));
  ...
  const overallScore = useMemo(() => {
    let finalScore = Math.round(spaceMetrics.score * 0.6 + timeMetrics.score * 0.4);
    // 40-point hard cap logic
    if ((spaceMetrics.hasSevereClash || timeMetrics.score < 25) && finalScore > 40) {
      finalScore = 40;
    }
    return finalScore;
  ...
  const advice = useMemo(() => {
    ...
    if (spaceMetrics.hasSevereClash) {
      return {
        ...
        actionLabel: onApplyAction ? '敦賀を経由する迂回ルートを挿入' : 'この移動をシミュレーターで詳細調整する',
    ...
    if (timeMetrics.score < 50 || humanMetrics.score < 50) {
      ...
      if (timeMetrics.riskFactors.length > 0) {
        timeIssues.push(`宇宙タイミングの乱れ（${timeMetrics.riskFactors.join('・')}）`);
      }
      if (simulatedAns > 60) {
      ...
      return {
        ...
        actionLabel: timeMetrics.bestAlternativeDate ? `${timeMetrics.bestAlternativeDate} に出発日を変更` : '生体回復アドバイスを表示',
        actionType: timeMetrics.bestAlternativeDate ? 'DATE' as const : 'REST' as const,
    ...
    return {
      ...
      solution: '空間方位、出発タイミング、そしてご自身の心身状態が完全にシンクロしています。安心してそのまま計画を実行してください。',
      actionLabel: null,
      actionType: null
    };
  ...
  return (
    <div className="bg-zinc-950/70 border border-zinc-900 rounded-[2rem] p-6 backdrop-blur-md relative overflow-hidden shadow-2xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b border-zinc-900/60">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <span className="text-[9px] font-mono text-zinc-500 uppercase block">総合シンクロ指数</span>
            <div className="flex items-baseline gap-1">
              <span className={`text-2xl font-black font-mono tracking-tighter ${
                overallScore > 70 ? 'text-emerald-400' : overallScore > 40 ? 'text-amber-400' : 'text-red-400 animate-pulse'
              }`}>
                {overallScore}
              </span>
          ...
          <div className={`px-2.5 py-1 text-[10px] font-bold font-mono tracking-widest uppercase border rounded-md ${
            overallScore > 70 
            ...
            {overallScore > 70 ? 'EXCELLENT' : overallScore > 40 ? 'WARNING' : 'CRITICAL'}
          </div>
        </div>
      </div>
        ...
        <div className="bg-black/30 border border-zinc-900/50 rounded-xl p-4 flex flex-col justify-between">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className={`text-sm font-mono font-bold ${isTenLow ? 'text-red-400' : 'text-fuchsia-400'}`}>
                {timeMetrics.score}%
              </span>
            </div>
          </div>
        ...
        <div className="bg-black/30 border border-zinc-900/50 rounded-xl p-4 flex flex-col justify-between">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className={`text-sm font-mono font-bold ${isChiLow ? 'text-red-400' : 'text-emerald-400'}`}>
                {spaceMetrics.score}%
              </span>
            </div>
          </div>
        ...
        <div className="bg-black/30 border border-zinc-900/50 rounded-xl p-4 flex flex-col justify-between">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className={`text-sm font-mono font-bold ${isJinLow ? 'text-red-400' : 'text-teal-400'}`}>
                {humanMetrics.score}%
              </span>
            </div>
          </div>
      ...
      <div className={`border p-4 rounded-xl flex gap-3 text-xs leading-relaxed ${
        ...
        <div className="mt-0.5 shrink-0">
          <AlertTriangle size={16} className={
            advice.severity === 'high' ? 'text-red-500' : advice.severity === 'medium' ? 'text-amber-500' : 'text-emerald-500'
          } />
        </div>
        <div className="flex-1 flex flex-col gap-2">
          <div>
            <span className="font-bold block mb-1">
              {advice.severity === 'high' ? '【警告】地脈エネルギーの衝突' : advice.severity === 'medium' ? '【調整推奨】天・人の不整合' : '【調和】完全整合化フェーズ'}
            </span>
          ...
          {advice.actionLabel && (onApplyAction || advice.actionType === 'NAVIGATE') && (
              ...
              <button
                onClick={() => {
                  if (advice.actionType === 'NAVIGATE') {
                    ...
                    const dateStr = step ? step.departureDate : new Date().toISOString().slice(0, 10);
                    ...
                    if (step) {
                      const bearing = getBearing(step.fromLat, step.fromLon, step.toLat, step.toLon);
