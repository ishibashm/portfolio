...
import { 
  Activity, Shield, CloudLightning, Compass, BrainCircuit, RefreshCcw, 
  TerminalSquare, BookOpen, AlertTriangle, Navigation, Orbit, Zap, 
  TrendingUp, Sparkles, Layers, Eye, Moon, Sun, Flame, Award, ChevronDown, ChevronUp,
  Star, CalendarDays, Rocket, Clock, ShieldCheck
} from "lucide-react";
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-extrabold bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 bg-clip-text text-transparent flex items-center gap-3">
            ...
            エージェンシー・メタフィジカル・システム (Agency Metaphysical System)
          </h1>
          <p className="text-gray-400 mt-2 text-sm">次善行動（NBA）意思決定エンジンと天体環境テレメトリ</p>
        </div>
      ...
      <div className="space-y-3 mb-6">
        {data?.macro.streams?.isVoidTime && (
          ...
          >
            ...
            <div className="text-xs md:text-sm">
              <span className="font-extrabold tracking-wide mr-2">⚠️ VOID TIME ACTIVE (天中殺/空亡期間)</span>
              <span className="text-amber-400/80">ユーザー空亡の十二支（戌亥）と現在の時盤が一致しています。能動的行動のQ値が補正されます。</span>
            </div>
          </motion.div>
        )}
        ...
        {data?.macro.streams?.isConflictDay && (
          ...
          >
            ...
            <div className="text-xs md:text-sm">
              <span className="font-extrabold tracking-wide mr-2">⚠️ CONFLICT DAY DETECTED (日支・年支相冲)</span>
              <span className="text-red-400/80">現在の地支が日主の地支と反発。守備的行動（ABORT_AND_SHIELD）の優先度が引き上げられています。</span>
            </div>
          </motion.div>
        )}
      </div>
            ...
            >
              ...
              <div className="p-6 rounded-[2rem] bg-white/5 border border-white/10 backdrop-blur-md flex flex-col items-center justify-center relative overflow-hidden shadow-xl">
                <div className="absolute top-0 left-0 p-4 w-full flex justify-between items-center">
                  <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">統合リスクエンジン (Unified Risk Engine)</span>
                  <span className={`px-2 py-0.5 rounded-full text-[8px] font-bold ${getRiskBg(unifiedRisk)} ${getRiskColor(unifiedRisk)}`}>
                    {unifiedRisk < 30 ? "低 (LOW)" : unifiedRisk < 60 ? "警戒 (ELEVATED)" : "危機 (CRITICAL)"}
                  </span>
                </div>
                ...
                <div className="relative w-36 h-36 flex items-center justify-center mt-6 mb-4">
                  ...
                  <div className="absolute flex flex-col items-center justify-center">
                    <span className="text-3xl font-black tracking-tight">{unifiedRisk.toFixed(1)}</span>
                    <span className="text-[9px] text-gray-400 uppercase tracking-widest font-bold">リスク指数</span>
                  </div>
                </div>
              ...
              <div className="p-6 rounded-[2rem] bg-white/5 border border-white/10 backdrop-blur-md flex flex-col justify-between shadow-xl">
                <div>
                  <div className="flex items-center gap-2.5 mb-4 border-b border-white/5 pb-3">
                    ...
                    <div>
                      <h3 className="font-bold text-sm text-orange-100">宇宙天気 (Space Weather)</h3>
                      <p className="text-[9px] text-gray-500">NOAAリアルタイム太陽宇宙天気フィード</p>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-9 gap-1 h-2">
                        {Array.from({ length: 9 }).map((_, i) => {
                          ...
                          if (active) {
                            color = i < 3 ? "bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.4)]" : i < 6 ? "bg-amber-500 shadow-[0_0_5px_rgba(245,158,11,0.4)]" : "bg-red-500 shadow-[0_0_5px_rgba(239,68,68,0.4)]";
                          }
                          return <div key={i} className={`h-full rounded-sm ${color} transition-all`} />;
                        })}
                      </div>
                    </div>
              ...
              <div className="p-6 rounded-[2rem] bg-white/5 border border-white/10 backdrop-blur-md flex flex-col justify-between shadow-xl">
                <div>
                  <div className="flex items-center gap-2.5 mb-4 border-b border-white/5 pb-3">
                    ...
                    <div>
                      <h3 className="font-bold text-sm text-blue-100">金融市場 (Market Economics)</h3>
                      <p className="text-[9px] text-gray-500">Yahoo Finance市場リスク指標</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-center text-xs p-3 rounded-xl bg-black/30 border border-white/5">
                      <div>
                        <p className="text-gray-400 font-medium">推定クレジットスプレッド</p>
                    ...
                    {data.macro.streams?.macroEconomics?.isMocked && (
                      <div className="flex items-center gap-1.5 text-[9px] text-gray-500 bg-white/5 p-2 rounded-lg border border-white/5">
                        ...
                        <span>APIがオフラインまたは未設定です。フォールバックシミュレーションがアクティブです。</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="p-6 rounded-[2rem] bg-white/5 border border-white/10 backdrop-blur-md flex flex-col justify-between shadow-xl">
                <div>
                  <div className="flex items-center gap-2.5 mb-4 border-b border-white/5 pb-3">
                    ...
                    <div>
                      <h3 className="font-bold text-sm text-indigo-100">重力・潮汐力 (Lunar Gravity)</h3>
                      <p className="text-[9px] text-gray-500">天体物理エンジン軌道計算</p>
                    </div>
                  </div>
              ...
              <div className="md:col-span-2 p-6 rounded-[2rem] bg-gradient-to-br from-emerald-500/10 to-transparent border border-emerald-500/20 backdrop-blur-md flex flex-col justify-between shadow-xl">
                <div>
                  <div className="flex items-center gap-2.5 mb-4 border-b border-emerald-500/10 pb-3">
                    ...
                    <div>
                      <h3 className="font-bold text-sm text-emerald-100">バイオテレメトリ (Oura Biometrics)</h3>
                      <p className="text-[9px] text-emerald-500/80">マイクロレベル自律神経負荷と生体パラメーターストリーム</p>
                    </div>
                  </div>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="p-3.5 rounded-2xl bg-black/30 border border-white/5">
                      <p className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">心拍変動 (Heart Rate Variability - HRV)</p>
                      <div className="flex justify-between items-baseline mt-1.5">
                        <span className="text-2xl font-black text-white">{data.micro.hrv}</span>
                        <span className="text-[10px] text-emerald-400 font-medium">最適 (Optimal)</span>
                      </div>
                      <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden mt-2">
                        <div className="h-full bg-emerald-400" style={{ width: `${data.micro.hrv}%` }} />
                      </div>
                    </div>

                    <div className="p-3.5 rounded-2xl bg-black/30 border border-white/5">
                      <p className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">睡眠回復指標 (Sleep Quality Index - GSR)</p>
                      <div className="flex justify-between items-baseline mt-1.5">
                        <span className="text-2xl font-black text-white">{data.micro.gsr}</span>
                        <span className="text-[10px] text-emerald-400 font-medium">良好 (Recharged)</span>
                      </div>
                      <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden mt-2">
                        <div className="h-full bg-emerald-400" style={{ width: `${data.micro.gsr}%` }} />
                      </div>
                    </div>

                    {data.micro.ansLoad !== undefined && (
                      <div className="p-3.5 rounded-2xl bg-black/30 border border-white/5">
                        <p className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">自律神経ストレス負荷 (ANS Load)</p>
                        <div className="flex justify-between items-baseline mt-1.5">
                          <span className="text-2xl font-black text-white">{data.micro.ansLoad.toFixed(0)}</span>
                          <span className={`text-[10px] font-medium ${data.micro.ansLoad > 60 ? 'text-red-400' : 'text-amber-400'}`}>
                            {data.micro.ansLoad > 60 ? '上昇 (Elevated)' : '中程度 (Moderate)'}
                          </span>
                        </div>
                        <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden mt-2">
                          <div 
            ...
            >
              ...
              <div className="p-5 rounded-2xl bg-gradient-to-r from-blue-500/10 via-indigo-500/5 to-transparent border border-white/10 backdrop-blur-md text-xs md:text-sm text-gray-300 leading-relaxed shadow-lg">
                ...
                <p className="mb-3 text-[11px] leading-relaxed text-gray-400">
                  これらのシステムは、<strong>「何を観測して変化を予測するか」</strong>というロジックの違いによって、大きく3つのグループに分類されます。宇宙や人間の状態を多角的にモデリングし、それぞれのAPIから取得したリアルタイムデータを統合して意思決定エンジンへフィードします。
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3 text-[11px] text-gray-400 border-t border-white/5 pt-3">
                  <div>
                    <span className="font-bold text-indigo-300 block mb-1">【A. 天体観測系】</span>
                    実際の空 of 星や計算上の天体配置（アスペクト等）から環境ステータスを座標軸として読み解きます。（DivineAPI / AstrologyAPI / VedAstro）
                  </div>
                  <div>
                    <span className="font-bold text-amber-300 block mb-1">【B. 暦・数理サイクル系】</span>
                    星を直接見ず、時間そのものが持つ周期（十干十二支・九星・数秘等）の干渉パターンを計算式で測ります。（Luck Pillars / Chinese Metasoft / KlavisAI MCP / DivineAPI）
                  </div>
                  <div>
                    <span className="font-bold text-purple-300 block mb-1">【C. 偶然・シンクロ系】</span>
                    偶然起きた出来事に全体の状況が縮小投影される「共時性（シンクロニシティ）」の原理に基づき変化の相を特定します。（RoxyAPI / strobus/i-ching / DivineAPI）
                  </div>
                </div>
              </div>
                ...
                <div className="p-6 rounded-[2rem] bg-gradient-to-b from-indigo-500/10 to-transparent border border-indigo-500/20 backdrop-blur-md flex flex-col justify-between shadow-xl relative overflow-hidden">
                  ...
                  <div>
                    <div className="flex items-center gap-2.5 mb-4 border-b border-indigo-500/25 pb-3">
                      ...
                      <div>
                        <h3 className="font-bold text-sm text-indigo-100">A. 天体観測系</h3>
                        <p className="text-[9px] text-indigo-300">実在の星の軌道と角度をモデル化</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                            ...
                            data?.macro.streams?.metaphysical?.astrologyApi?.aspects?.map((asp, idx) => (
                              <div key={idx} className="flex justify-between items-center text-gray-300 py-0.5">
                                ...
                                <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${
                                  asp.quality === 'harmonious' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/25' :
                                  asp.quality === 'discordant' ? 'bg-red-500/20 text-red-400 border border-red-500/25' :
                                  'bg-white/5 text-gray-400'
                                }`}>
                                  {asp.quality === 'harmonious' ? '調和 (吉)' : asp.quality === 'discordant' ? '不調和 (凶)' : '中立'}
                                </span>
                              </div>
                            ))
                          ) : (
                            <p className="text-gray-500 italic">アスペクトデータを読み込み中...</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                        <div className="flex justify-between items-baseline mb-2">
                          ...
                          <span className="text-[8px] text-gray-500">VedAstro MCP</span>
                        </div>
                      ...
                      {data?.macro?.streams?.metaphysical?.ziWeiDouShu && (
                        <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                          <div className="flex justify-between items-baseline mb-2">
                            <p className="text-[10px] text-indigo-400 uppercase font-bold tracking-widest flex items-center gap-1">
                              <Star className="w-3 h-3" /> 紫微斗数 (Zi Wei Dou Shu)
                            </p>
                            <span className="text-[8px] text-gray-500">Calendar Stars</span>
                          </div>
                      ...
                      {data.macro.streams?.metaphysical?.humanDesign && (
                        <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                          <div className="flex justify-between items-baseline mb-2">
                            <p className="text-[10px] text-indigo-400 uppercase font-bold tracking-widest flex items-center gap-1">
                              <Layers className="w-3 h-3" /> ヒューマンデザイン (Human Design)
                            </p>
                            <span className="text-[8px] text-gray-500">Synthesis Engine</span>
                          </div>
                          <div className="space-y-1.5 text-xs text-gray-300">
                            <div className="flex justify-between">
                ...
                <div className="p-6 rounded-[2rem] bg-gradient-to-b from-amber-500/10 to-transparent border border-amber-500/20 backdrop-blur-md flex flex-col justify-between shadow-xl relative overflow-hidden">
                  ...
                  <div>
                    <div className="flex items-center gap-2.5 mb-4 border-b border-amber-500/25 pb-3">
                      ...
                      <div>
                        <h3 className="font-bold text-sm text-amber-100">B. 暦・数理サイクル系</h3>
                        <p className="text-[9px] text-amber-300">時間周期・数理振動数から環境変数を計算</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                        <div className="flex justify-between items-baseline mb-2">
                          <p className="text-[10px] text-amber-400 uppercase font-bold tracking-widest flex items-center gap-1">
                            <Flame className="w-3 h-3" /> 四柱推命 / 大運 (BaZi / Luck Pillars)
                          </p>
                          <span className="text-[8px] text-gray-500">Luck Pillars / KlavisAI MCP</span>
                        </div>
                        <div className="space-y-2 text-xs text-gray-300">
                          <div className="flex justify-between items-baseline">
                            <span className="text-gray-500 text-[9px] uppercase">日主 (Day Master)</span>
                            <span className="font-bold text-white text-sm bg-white/5 px-2 py-0.5 rounded border border-white/5">
                              {data.macro.streams?.personalBazi?.summary?.dayMaster ? `${data.macro.streams.personalBazi.summary.dayMaster} (${data.macro.streams.personalBazi.summary.dayMasterWuxing}) - ${data.macro.streams.personalBazi.summary.strength}` : '庚金 (Yang Metal) - Strong'}
                            </span>
                          </div>
                          
                          {data.macro.streams?.metaphysical?.chineseMetasoft?.daYunPillar ? (
                            <div>
                            ...
                            <p className="text-gray-500 italic text-[10px]">Natal Luck Cycle computed locally.</p>
                          )}
                          
                          <div>
                            <span className="text-gray-500 block text-[9px] uppercase mb-1">奇門遁甲 アクティブな門</span>
                            <div className="flex justify-between items-center p-2 rounded-lg bg-white/5 border border-white/5">
                              <div>
                                <span className="font-bold text-white">{data.macro.streams?.metaphysical?.chineseMetasoft?.qiMenGate?.name ?? '生門 (Sheng Men)'}</span>
                                <span className="text-[9px] text-gray-400 ml-1.5">方位: {data.macro.streams?.metaphysical?.chineseMetasoft?.qiMenGate?.direction ?? 'NE'}</span>
                              </div>
                              <span className={`px-2 py-0.5 rounded text-[8px] font-bold ${
                                data.macro.streams?.metaphysical?.chineseMetasoft?.qiMenGate?.status === 'Auspicious' ? 'bg-emerald-500/20 text-emerald-400' :
                                data.macro.streams?.metaphysical?.chineseMetasoft?.qiMenGate?.status === 'Inauspicious' ? 'bg-red-500/20 text-red-400' :
                                'bg-white/5 text-gray-400'
                              }`}>
                                {data.macro.streams?.metaphysical?.chineseMetasoft?.qiMenGate?.status ?? 'Auspicious'}
                              </span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                        ...
                        <div className="text-xs text-gray-300">
                          <div className="flex justify-between items-baseline mb-1">
                            <span className="text-gray-500 text-[9px] uppercase">ライフパスナンバー</span>
                            <span className="text-lg font-black text-amber-400 font-mono">
                              {data.macro.streams?.metaphysical?.divineApi?.numerology?.lifePathNumber ?? '8'}
                            </span>
                          </div>
                          <p className="text-[10px] text-gray-400 leading-relaxed">
                            {data.macro.streams?.metaphysical?.divineApi?.numerology?.description ?? 'The Achiever - Ambition, material success, authority, and karma.'}
                          </p>
                        </div>
                      </div>
                      ...
                      {data.macro.streams?.metaphysical?.nineStarKi && (
                        <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                          <div className="flex justify-between items-baseline mb-2">
                            <p className="text-[10px] text-amber-400 uppercase font-bold tracking-widest flex items-center gap-1">
                              <Layers className="w-3 h-3" /> 九星気学 (Nine Star Ki)
                            </p>
                            <span className="text-[8px] text-gray-500">9-Cycle Magic Square</span>
                          </div>
                          <div className="space-y-1.5 text-xs text-gray-300">
                            <div className="flex justify-between">
                      ...
                      {data.macro.streams?.metaphysical?.mayaTzolkin && (
                        <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                          <div className="flex justify-between items-baseline mb-2">
                            <p className="text-[10px] text-amber-400 uppercase font-bold tracking-widest flex items-center gap-1">
                              <Moon className="w-3 h-3" /> マヤ暦 (Tzolk&apos;in)
                            </p>
                            <span className="text-[8px] text-gray-500">Sacred 260 Kin</span>
                          </div>
                          <div className="space-y-1.5 text-xs text-gray-300">
                      ...
                      {data.macro.streams?.metaphysical?.kabbalahTree && (
                        <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                          <div className="flex justify-between items-baseline mb-2">
                            ...
                            <span className="text-[8px] text-gray-500">Resonance Path</span>
                          </div>
                          <div className="space-y-1.5 text-xs text-gray-300">
                            <div className="flex justify-between">
                  ...
                  <div className="flex flex-col gap-2 mt-4">
                    ...
                    >
                      ...
                      {showNatalBazi ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                    </button>
                    <button 
                    ...
                    >
                      ...
                      {showEnvBazi ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                    </button>
                  </div>
                </div>
                ...
                <div className="p-6 rounded-[2rem] bg-gradient-to-b from-purple-500/10 to-transparent border border-purple-500/20 backdrop-blur-md flex flex-col justify-between shadow-xl relative overflow-hidden">
                  ...
                  <div>
                    <div className="flex items-center gap-2.5 mb-4 border-b border-purple-500/25 pb-3">
                      ...
                      <div>
                        <h3 className="font-bold text-sm text-purple-100">C. 偶然・シンクロ系</h3>
                        <p className="text-[9px] text-purple-300">共時性の原理を基に「変化のフェーズ」を特定</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                        <div className="flex justify-between items-baseline mb-2">
                          ...
                          <span className="text-[8px] text-gray-500">RoxyAPI / strobus/i-ching</span>
                        </div>
                        
                        {data.macro.streams?.metaphysical?.roxyApi?.ichingCast ? (
                                ...
                                {data?.macro.streams?.metaphysical?.roxyApi?.ichingCast?.lines?.slice()?.reverse()?.map((line, idx) => {
                                  ...
                                  return (
                                    <div key={idx} className="flex items-center gap-2">
                                      {isYang ? (
                                        <div className={`w-14 h-2 bg-indigo-400 rounded-sm relative ${isChanging ? 'shadow-[0_0_8px_rgba(239,68,68,0.7)] bg-rose-400' : 'shadow-[0_0_5px_rgba(129,140,248,0.4)]'}`} />
                                      ) : (
                                        <div className="w-14 h-2 flex justify-between">
                                          <div className={`w-[45%] h-full bg-indigo-500/60 rounded-sm ${isChanging ? 'bg-rose-500/60 shadow-[0_0_5px_rgba(239,68,68,0.4)]' : ''}`} />
                                          <div className={`w-[45%] h-full bg-indigo-500/60 rounded-sm ${isChanging ? 'bg-rose-500/60 shadow-[0_0_5px_rgba(239,68,68,0.4)]' : ''}`} />
                                        </div>
                                      )}
                                      {isChanging && <span className="text-[7px] text-red-400 font-black">●</span>}
                                    </div>
                                  );
                                })}
                              </div>
                          ...
                          <p className="text-gray-500 italic">Cast data unavailable.</p>
                        )}
                      </div>
                      ...
                      <div className="p-3 rounded-xl bg-black/40 border border-white/5 flex flex-col">
                        <div className="flex justify-between items-baseline mb-2">
                          <p className="text-[10px] text-purple-400 uppercase font-bold tracking-widest mb-3 flex items-center gap-1">
                            <Eye className="w-3 h-3" /> タロット展開 (Tarot Draw)
                          </p>
                          <span className="text-[8px] text-gray-500">DivineAPI</span>
                        </div>
                        ...
                        {data?.macro.streams?.metaphysical?.divineApi?.tarot ? (
                          ...
                          <p className="text-gray-500 italic">Tarot draw loading...</p>
                        )}
                      </div>
                        ...
                        <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                          <div className="flex justify-between items-baseline mb-2">
                            <p className="text-[10px] text-purple-400 uppercase font-bold tracking-widest flex items-center gap-1">
                              <Compass className="w-3 h-3" /> ジオマンシー (Geomancy)
                            </p>
                            <span className="text-[8px] text-gray-500">Medieval 16 Figures Cast</span>
                          </div>
            ...
            >
              ...
              <div className="md:col-span-2 md:row-span-2 p-8 rounded-[2rem] bg-gradient-to-br from-indigo-500/10 via-purple-500/5 to-transparent border border-indigo-500/20 backdrop-blur-md relative overflow-hidden shadow-xl flex flex-col justify-between group">
                <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-25 transition-all duration-500">
                  <Compass className="w-36 h-36 text-indigo-400 rotate-12" strokeWidth={1} />
                </div>
                ...
                <div className="relative z-10 h-full flex flex-col justify-between">
                  <div>
                    ...
                    <h2 className="text-4xl md:text-5xl font-black text-white mb-2 tracking-tight">
                      {data.nba.actionResult.suggestedAction === 'PREPARE_AND_WAIT' && data.nba.actionResult.expectedReward < 0 
                        ? '警戒待機（マイナス回避）' 
                    ...
                    <div className="mt-6 p-4 rounded-2xl bg-indigo-950/40 border border-indigo-500/20 backdrop-blur-sm shadow-inner">
                      <div className="flex justify-between items-end mb-3">
                        <h4 className="text-[10px] text-indigo-300 uppercase font-bold tracking-widest flex items-center gap-2">
                          <BrainCircuit className="w-3.5 h-3.5" /> 行動ポリシー選択確率マトリクス
                        </h4>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-6">
                    <div className="p-4 rounded-2xl bg-black/40 border border-white/5 shadow-inner">
                      <p className="text-gray-400 text-xs uppercase font-medium mb-1">確信度 (Confidence)</p>
                      <p className="text-2xl font-bold text-emerald-400">{(data.nba.actionResult.confidence * 100).toFixed(1)}%</p>
                    </div>
                    <div className="p-4 rounded-2xl bg-black/40 border border-white/5 shadow-inner">
                      <p className="text-gray-400 text-xs uppercase font-medium mb-1">ポリシーの種類 (Policy Type)</p>
              ...
              <div className="md:col-span-2 p-6 rounded-[2rem] bg-white/5 border border-white/10 backdrop-blur-md shadow-xl flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-3 mb-6 border-b border-white/5 pb-3">
                    ...
                    <h3 className="font-bold text-sm text-blue-100">FQI 状態ベクトルパラメーター (State Vector)</h3>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3.5 rounded-xl bg-black/30 border border-white/5 overflow-hidden">
                      <p className="text-[10px] text-gray-500 uppercase font-bold tracking-wider mb-1">ANS自律神経負荷</p>
                      <p className="text-lg font-mono font-black text-white">{Number(data.nba.stateVector.ansLoad).toFixed(2)}</p>
                    </div>
                    <div className="p-3.5 rounded-xl bg-black/30 border border-white/5 overflow-hidden">
                      <p className="text-[10px] text-gray-500 uppercase font-bold tracking-wider mb-1">シールド容量 (睡眠スコア)</p>
                      <p className="text-lg font-mono font-black text-white">{Number(data.nba.stateVector.shieldCapacity).toFixed(2)}</p>
                    </div>
                    <div className="p-3.5 rounded-xl bg-black/30 border border-white/5 overflow-hidden">
                      <p className="text-[10px] text-orange-500/70 uppercase font-bold tracking-wider mb-1">環境リスクスコア (Risk)</p>
                      <p className="text-lg font-mono font-black text-white">{Number(data.nba.stateVector.environmentalRisk || 50).toFixed(2)}</p>
                    </div>
                    <div className="p-3.5 rounded-xl bg-black/30 border border-white/5 overflow-hidden">
                      <p className="text-[10px] text-gray-500 uppercase font-bold tracking-wider mb-1">太陽黄経位相 (Solar Phase)</p>
                      <p className="text-lg font-mono font-black text-white">{Number(data.nba.stateVector.solarPhase).toFixed(2)}°</p>
                    </div>
                  </div>
                </div>

                <div className="p-3 rounded-xl bg-black/30 border border-white/5 mt-4 flex justify-between items-center text-xs">
                  <span className="text-gray-400">期待されるポリシー報酬 (Expected Reward)</span>
                  <span className={`font-mono font-bold ${data.nba.actionResult.expectedReward < 0 ? 'text-red-400' : 'text-emerald-400'}`}>
                    {data.nba.actionResult.expectedReward > 0 ? '+' : ''}{data.nba.actionResult.expectedReward.toFixed(2)}
                  </span>
                </div>
              </div>
              ...
              <div className="md:col-span-4 p-6 rounded-[2rem] bg-indigo-900/20 border border-indigo-500/20 backdrop-blur-md shadow-xl mt-2">
                <div className="flex flex-col md:flex-row md:items-center gap-3 mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-bold text-sm text-indigo-100">1年間の最適行動予測 (1-Year Action Forecast)</h3>
                  </div>
                  <div className="ml-auto flex flex-col items-end">
                    <span className="text-[10px] text-indigo-300 bg-indigo-500/10 px-2 py-1 rounded">※現在のシールド値を基準とした予測</span>
                  </div>
                </div>

                <div className="mb-6 p-3 rounded-xl bg-black/30 border border-white/5 text-xs text-gray-300 leading-relaxed">
                  <p className="font-bold text-indigo-300 mb-1 flex items-center gap-1"><ShieldCheck className="w-3.5 h-3.5" /> シールド値（防御力）と計算ロジックについて</p>
                  <p>
                    このエンジンでは、<strong>「シールド（精神的・肉体的・資金的余裕）」</strong>が最も重要なリソースとして計算されます。
                    通常時、移住（実行前進）にはシールドを<span className="text-red-400">約15%</span>消費しますが、天中殺などの悪環境下での強行（浄化移住）には<span className="text-red-400">約40%</span>のシールドを消費するモデリングになっています。<br/>
                    つまり、どんなに星回りが悪くても<strong>「シールドがほぼMAXであれば、ダメージを吸収して行動可能」</strong>とAIは判断し、逆にシールドが枯渇していると、平時であっても「待機・回復」を強制指示します。
                  </p>
                </div>
                ...
                <div className="h-64 w-full mt-4 relative">
                  {forecastLoading ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/20 rounded-xl">
                      ...
                      <p className="text-xs text-indigo-300">天体・暦推移をシミュレーション中...</p>
                    </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <ComposedChart data={forecastData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <XAxis dataKey="name" stroke="rgba(255,255,255,0.3)" fontSize={10} tickMargin={10} />
                          ...
                          if (active && payload && payload.length) {
                            return (
                              <div className="bg-black/80 border border-white/10 p-3 rounded-xl backdrop-blur-md shadow-xl text-xs">
                                <p className="font-bold text-white mb-2 border-b border-white/10 pb-1">{label} <span className="text-gray-500 font-mono text-[9px] ml-1">{new Date(payload[0].payload.date).toLocaleDateString()}</span></p>
                                <div className="flex items-center justify-between gap-4 mb-1">
                                  <span className="text-emerald-400 font-bold">シールド予測残量:</span>
                                  <span className="text-white font-mono">{payload[1].value.toFixed(0)}</span>
                                </div>
                                <div className="flex items-center justify-between gap-4 mb-2">
                                  <span className="text-red-400 font-bold">環境負荷コスト:</span>
                                  <span className="text-white font-mono">{payload[0].value.toFixed(0)}</span>
                                </div>
                                <div className="bg-white/5 p-1.5 rounded text-center">
                                  <span className="text-indigo-300 font-bold">AI推奨: {payload[0].payload.action}</span>
                                </div>
                                {payload[0].payload.isVoidTime && (
                                  <div className="mt-1 text-center">
                                    <span className="text-red-400 text-[9px] font-bold">⚠️ 天中殺/空亡期間</span>
                                  </div>
                                )}
                              </div>
                            );
                        ...
                        <Area type="monotone" dataKey="envCost" stroke="#f87171" fillOpacity={1} fill="url(#colorEnv)" name="環境負荷コスト" />
                        <Line type="monotone" dataKey="shield" stroke="#34d399" strokeWidth={3} dot={{ r: 4, fill: '#34d399', stroke: '#000', strokeWidth: 2 }} activeDot={{ r: 6 }} name="シールド予測残量" />
                      </ComposedChart>
                    </ResponsiveContainer>
                  ) : (
                    <div className="flex items-center justify-center h-full text-gray-500 text-xs">データがありません</div>
                  )}
                </div>
              </div>
              ...
              {data.nba.actionResult.logicTrace && (
                <div className="md:col-span-4 p-6 rounded-[2rem] bg-black/40 border border-white/5 backdrop-blur-md shadow-xl">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="font-bold text-sm text-gray-100">System Logic Trace (因果推論ログ)</h3>
                  </div>
                  <div className="bg-black/60 rounded-xl p-4 font-mono text-xs text-green-400/80 overflow-y-auto max-h-[220px] custom-scrollbar border border-white/5 shadow-inner">
                    {data.nba.actionResult.logicTrace.map((log: string, idx: number) => (
                      <div key={idx} className="mb-1.5 last:mb-0 break-all leading-relaxed">
                        <span className="text-gray-600 select-none mr-2 font-bold">{String(idx + 1).padStart(2, '0')}</span>
                        {log}
                      </div>
                    ))}
                  </div>
