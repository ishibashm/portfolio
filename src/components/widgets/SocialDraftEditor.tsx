...
export const SocialDraftEditor = ({ draftText }: SocialDraftEditorProps) => {
  const [text, setText] = useState(
    draftText ||
      "Apple's latest earnings report shows strong momentum. #Tech #Finance",
  );
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Agent-Reach & dexter-jp pipeline
          </p>
        </div>
      ...
      <div className="flex-1 flex flex-col gap-3">
        ...
        <div className="flex justify-between items-center text-xs">
          <div className="text-slate-400 flex items-center gap-2">
            ...
            {text.length > 0 && (
              <span className="text-emerald-400">Persona check: PASS</span>
            )}
          </div>
          <div
            className={`${text.length % maxLength > maxLength - 20 ? "text-amber-400" : "text-slate-400"}`}
          >
