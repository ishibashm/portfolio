...
export default function OmniPipelineWidget() {
  ...
  const handleLaunchBrowser = async () => {
    ...
    try {
      ...
      addLog('completed', 'ブラウザ起動完了。収集準備が整ったら次へ進んでください。');
    } catch (err: any) {
  ...
  const handleStartScraping = async () => {
    ...
    try {
      ...
      const authorHandle = parts.length > 0 && parts[0] !== 'home' ? parts[0] : 'nikkei'; 
      ...
      if (!fileRes.ok) {
        ...
        throw new Error(`収集データの読み込みに失敗しました (${targetJsonl} が見つかりません。別のURLをお試しください)。詳細: ${errData.detail || fileRes.statusText}`);
      }
      ...
      const analyzeRes = await fetch('/api/v1/analyzer/process', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ filename: targetJsonl, content: fileData.content }),
      });
    ...
    <div className="flex flex-col h-full bg-[#0f172a] border border-indigo-900/50 rounded-xl overflow-hidden shadow-2xl relative">
      ...
      <div className="px-5 py-4 bg-slate-900/90 border-b border-indigo-900/50 flex justify-between items-center z-10 relative">
        <h2 className="text-base font-bold flex items-center gap-2 text-white tracking-wide">
          <span className="text-indigo-400 text-xl">⚡</span> Omni-Research Pipeline
        </h2>
        {isProcessing && (
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar relative p-6 space-y-8">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-end border-b border-slate-700/50 pb-2">
            <h3 className="text-lg font-bold text-slate-200">1. データ収集 (Data Collection)</h3>
            <p className="text-xs text-slate-400">Xタイムラインの取得 または JSONLの手動アップロード</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-800/40 border border-slate-700 rounded-xl p-5 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-medium text-slate-500">ターゲットURL</label>
                <input 
                  type="text" 
                  value={url} 
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-slate-800/40 border border-slate-700 rounded-xl p-5 flex flex-col items-center justify-center gap-3">
                ...
                <h4 className="font-bold text-slate-300 text-sm">既存JSONLアップロード</h4>
                <button 
              ...
              <div className="flex-1 bg-slate-950 border border-slate-800 rounded-xl p-3 font-mono text-[10px] text-slate-400 h-40 flex flex-col relative">
                <div className="text-slate-500 mb-2 pb-1 border-b border-slate-800 flex justify-between shrink-0">
                  <span>実行ログ (Logs)</span>
                  {isProcessing && <span className="text-indigo-400 animate-pulse">Running...</span>}
                </div>
                <div className="flex flex-col gap-1 overflow-y-auto custom-scrollbar flex-1 pb-2">
                  {logs.length === 0 ? (
                    ...
                    logs.map((log) => (
                      <div key={log.id} className={`p-1 rounded ${
                        log.type === 'error' ? 'text-red-400 bg-red-950/20' : 
                        log.type === 'completed' ? 'text-emerald-400 bg-emerald-950/20' : 
        ...
        {analysisResult && (
          <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-end border-b border-slate-700/50 pb-2">
              <div>
                <h3 className="text-lg font-bold text-slate-200">2. 分析結果＆キーワード (Analysis & Extraction)</h3>
            ...
            <div className="bg-slate-900/50 rounded-xl border border-slate-700/50 p-4">
              <h4 className="text-xs font-bold text-slate-400 mb-3 uppercase tracking-widest">頻出キーワード (クリックで選択)</h4>
            ...
            {selectedKeywords.length > 0 && (
              <div className="border-2 border-orange-500/30 bg-orange-950/10 rounded-xl p-5 relative overflow-hidden">
                ...
                <h4 className="text-sm font-bold text-orange-400 flex items-center gap-2 mb-4">
                  <span className="text-xl">🔎</span> 選択されたキーワードを含むポスト
                </h4>
                  ...
                  {analysisResult.posts
                    ...
                    .map((p: any, idx: number) => (
                      <div key={idx} className="bg-slate-900/80 border border-slate-700/50 p-4 rounded-lg flex flex-col gap-2">
                        ...
                        <div className="text-sm text-slate-300 whitespace-pre-wrap leading-relaxed">{p.text}</div>
                        {p.media_paths && p.media_paths.length > 0 && (
                          <div className="flex gap-2 mt-2 overflow-x-auto pb-1 custom-scrollbar">
                            {p.media_paths.map((mp: string, midx: number) => (
                              <div key={midx} className="h-16 bg-slate-800 rounded px-3 py-1 text-[10px] flex flex-col justify-center text-slate-500 font-mono border border-slate-700 whitespace-nowrap">
                                <span>📎 添付画像</span>
                                <span className="text-indigo-400 truncate max-w-[150px]">{mp.split('/').pop() || mp}</span>
                              </div>
                            ))}
                          </div>
                        )}
          ...
          <div className="flex flex-col gap-4 animate-in fade-in duration-500">
             <div className="flex justify-between items-end border-b border-emerald-900/50 pb-2">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">特定された銘柄一覧（TradingViewウィジェットと自動同期されます）</p>
              </div>
              <button 
            ...
            <div className="flex flex-wrap gap-2 p-4 bg-emerald-950/10 rounded-xl border border-emerald-900/30">
              {globalWatchlist.map((ticker, i) => (
                <span key={i} className="px-3 py-1.5 bg-emerald-900/30 text-emerald-300 text-sm font-mono rounded border border-emerald-700/50 shadow-sm">
                  {ticker}
                </span>
              ))}
            </div>
