    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-slate-900/95 border border-slate-700/80 p-3 rounded-lg shadow-[0_0_15px_rgba(59,130,246,0.3)] backdrop-blur-md">
          <p className="text-slate-400 text-xs mb-2 font-mono">{format(parseISO(label), 'yyyy MMM dd')}</p>
          <div className="space-y-1 font-mono text-sm">
            <p className="text-blue-400 flex justify-between gap-4">
              <span>Upper (95%):</span> <span>${payload[0].payload.upper_bound_95.toFixed(2)}</span>
            </p>
            <p className="text-white font-bold flex justify-between gap-4 border-l-2 border-blue-500 pl-2">
              <span>Mean:</span> <span>${payload[0].payload.mean.toFixed(2)}</span>
            </p>
            <p className="text-blue-400/70 flex justify-between gap-4">
              <span>Lower (95%):</span> <span>${payload[0].payload.lower_bound_95.toFixed(2)}</span>
            </p>
          </div>
        </div>
      );
      ...
      <div className="flex justify-between items-start mb-2 px-2">
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <h4 className="text-2xl font-extrabold text-white tracking-tighter">{ticker}</h4>
          ...
          <p className="text-[11px] text-slate-400 mt-1 uppercase tracking-widest font-medium">30-Day Predictive Horizon</p>
        </div>
        <div className="flex flex-col items-end">
          <p className={`text-xl font-mono font-bold tracking-tight ${isPositive ? 'text-emerald-400' : 'text-rose-400'}`}>
            {isPositive ? '+' : ''}{changePct.toFixed(2)}%
          </p>
          <p className="text-[10px] text-slate-500 uppercase tracking-widest">Expected Delta</p>
        </div>
      </div>
