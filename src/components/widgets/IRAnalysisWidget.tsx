...
export default function IRAnalysisWidget({
  ticker,
  companyName,
}: {
  ticker?: string;
  ...
  const [activeTab, setActiveTab] = useState<"edinet" | "visualize">(
    "visualize",
  );
  ...
  const fetchDBFinancials = (searchTicker: string, searchName?: string) => {
    fetch(
      `/api/v1/finance/summary/${encodeURIComponent(searchTicker)}?name=${encodeURIComponent(searchName || "")}`,
    )
      .then((res) => res.json())
      .then((data) => {
        if (!data.error) {
  ...
  useEffect(() => {
    ...
    setIsLoadingFinance(true);
    fetch(
      `/api/v1/finance/summary/Unknown?name=${encodeURIComponent(selectedCompany)}`,
    )
      .then((res) => res.json())
      .then((data) => {
        if (!data.error) {
          ...
          <div className="flex flex-col h-full p-6 overflow-auto custom-scrollbar bg-slate-950">
            ...
            {financialData ? (
              <div className="flex flex-col gap-6 animate-in fade-in duration-500">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col items-center justify-center">
                    ...
                    <span className="text-2xl font-mono text-emerald-400">
                      {financialData.net_profit_rate !== null
                        ? `${financialData.net_profit_rate.toFixed(2)}%`
                        : "N/A"}
                    </span>
                  </div>
                  <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col items-center justify-center">
                    ...
                    <span className="text-2xl font-mono text-blue-400">
                      {financialData.operation_profit_rate !== null
                        ? `${financialData.operation_profit_rate.toFixed(2)}%`
                        : "N/A"}
                    </span>
                  </div>
                  <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col items-center justify-center">
                    ...
                    <span className="text-2xl font-mono text-indigo-400">
                      {financialData.ordinary_profit_rate !== null
                        ? `${financialData.ordinary_profit_rate.toFixed(2)}%`
                        : "N/A"}
                    </span>
                  </div>
                </div>
                ...
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl mt-4">
                  ...
                  <div className="space-y-4">
                    {renderBar(
                      "売上高",
                      financialData.net_sales || 0,
                      financialData.net_sales || 1,
                      "bg-slate-400",
                    )}
                    {renderBar(
                      "営業利益",
                      financialData.operating_income || 0,
                      financialData.net_sales || 1,
                      "bg-blue-500",
                    )}
                    {renderBar(
                      "経常利益",
                      financialData.ordinary_income || 0,
                      financialData.net_sales || 1,
                      "bg-indigo-500",
                    )}
                    {renderBar(
                      "純利益",
                      financialData.net_income || 0,
                      financialData.net_sales || 1,
                      "bg-emerald-500",
                    )}
                  </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-slate-500 gap-2">
                ...
                <p>
                  {!isLoadingFinance && companies.length > 0
                    ? "データが取得できませんでした。"
                    : "企業を選択するか、AI分析から企業を特定してください。"}
                </p>
                {!ticker && companies.length === 0 && (
                  <p className="text-xs text-slate-600 max-w-sm text-center mt-2">
                    ...
                    <br />
                    EDINET
                    API経由でバッチ(`import_financial_data.py`)を実行するか、左側のData
                    Analyzerから企業を特定してください。
                  </p>
                )}
              </div>
            )}
