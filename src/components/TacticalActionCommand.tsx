...
export function TacticalActionCommandComponent({
  kpIndex,
  ansLoad,
  isPersonalVoid,
  personalVoidZodiac,
  ...
  let colorClass = "text-emerald-600 border-emerald-200 bg-emerald-50";
  ...
  if (isPersonalVoid) {
    ...
    colorClass =
      "text-red-500 border-red-200 bg-red-50 shadow-[inset_0_0_80px_rgba(255,0,0,0.3)] animate-pulse border-2";
  ...
  return (
    <div
      className={`w-full max-w-4xl border p-3 flex flex-col gap-3 rounded-none relative overflow-hidden transition-all duration-500 ${colorClass}`}
    >
      ...
      <div className="mt-4 border-t border-current/30 pt-4 grid grid-cols-1 md:grid-cols-3 gap-3 text-current">
        <div className="bg-white/70 p-4 border border-current/20 flex flex-col items-center hover:bg-white/70 transition-colors">
          <span className="text-xl md:text-2xl font-black tracking-tighter uppercase italic">
            STAY
          </span>
          <span className="text-[10px] md:text-xs font-mono tracking-widest opacity-80 mt-1">
            01. Zero-Volt Sync
          </span>
        </div>
        <div className="bg-white/70 p-4 border border-current/20 flex flex-col items-center hover:bg-white/70 transition-colors">
