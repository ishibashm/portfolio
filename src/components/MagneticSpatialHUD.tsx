...
export function MagneticSpatialHUD({
  ...
  const noiseIntensity = useMemo(() => Math.max(0, (kpIndex - 2) / 7), [kpIndex]);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      ctx.strokeStyle = noiseIntensity > 0.4 ? "rgba(239, 68, 68, 0.5)" : "rgba(59, 130, 246, 0.3)";
      ctx.lineWidth = 1;
      
      for (let j = 0; j < 6; j++) {
        ctx.beginPath();
        const r = radius + Math.sin(time * 2 + j) * 5 * noiseIntensity;
        ctx.ellipse(centerX, centerY, r, r * 0.6, (angle + j * 30) * Math.PI / 180, 0, Math.PI * 2);
        ctx.stroke();
      }
  ...
  return (
    <div className="relative group cursor-help" style={{ width: size, height: size * 0.8 }}>
      <canvas 
        ref={canvasRef} 
        width={size} 
        height={size * 0.8}
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col items-center justify-center pointer-events-none select-none">
        ...
        <div className="flex gap-4 text-[7px] font-mono text-zinc-500 mt-0.5">
          ...
          <span className={kpIndex >= 4 ? "text-red-500" : "text-emerald-500"}>WTH: Kp{kpIndex.toFixed(1)}</span>
        </div>
      </div>
