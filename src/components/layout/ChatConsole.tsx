...
import { Send, Paperclip, TerminalSquare, Command, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
...
export const ChatConsole = ({ messages, sendMessage, isThinking }: ChatConsoleProps) => {
    ...
    <main className="flex-1 flex flex-col bg-[#0b1120] border-r border-slate-800 relative max-w-3xl min-w-[450px]">
      ...
      <div className="p-4 border-b border-slate-800/80 bg-[#0f172a]/90 backdrop-blur z-10 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">LangGraph Multi-Agent Runtime</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-[url('/grid.svg')] bg-center [mask-image:linear-gradient(180deg,white,rgba(255,255,255,0))]">
        ...
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-6 p-4 animate-in fade-in duration-500">
            ...
            <div className="text-center space-y-2">
              <h3 className="text-lg font-bold text-slate-200 tracking-wide">Omni-Terminal Agent</h3>
              <p className="text-sm text-slate-500 max-w-md mx-auto leading-relaxed">
                X(旧Twitter)の情報収集、企業分析、メディア・動画の収集とフレーム抽出をサポートします。<br/>以下の推奨アクションをクリックして開始してください。
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-2xl mt-4">
              <button 
                onClick={() => sendMessage("Xで『多摩川くん』という隠語が指す企業を特定し、業績を分析して")}
              ...
              >
                ...
                <div className="text-xs text-slate-300 leading-relaxed">Xで『多摩川くん』という隠語が指す企業を特定し、業績を分析して</div>
              </button>

              <button 
                onClick={() => sendMessage("指定したXの投稿URLから、画像をまとめてダウンロードして")}
              ...
              >
                ...
                <div className="text-xs text-slate-300 leading-relaxed">指定したXの投稿URLから、画像をまとめてダウンロードして</div>
              </button>
              ...
              <div className="text-left p-4 rounded-xl bg-slate-800/30 border border-slate-700/50 border-dashed relative overflow-hidden">
                ...
                <div className="text-[11px] text-slate-400 leading-relaxed">
                  <span className="text-slate-300 font-semibold mb-1 block">Xの画像/動画・フレームの保存先:</span>
                  収集したメディアや抽出した画像は標準で<br/>
                  <code className="bg-slate-900 px-1.5 py-0.5 rounded text-emerald-400 border border-slate-700 mx-0.5">api-gateway/media_output/frames/</code><br/>
                  等のバックエンド指定ディレクトリに自動保存されます。
                </div>
              </div>

              <button 
                onClick={() => sendMessage("このローカルの動画ファイル(MP4)から、指定した間隔で画像フレームを抽出して")}
              ...
              >
                ...
                <div className="text-xs text-slate-300 leading-relaxed">このローカルの動画ファイル(MP4)から、指定した間隔で画像フレームを抽出して</div>
              </button>
            </div>
          </div>
        )}
        ...
        {messages.map((msg, index) => (
          <div key={msg.id} className={`flex flex-col ${msg.type === 'thought' ? 'items-start' : 'items-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
      ...
      <div className="p-4 bg-[#0f172a] border-t border-slate-800/80">
        <form onSubmit={handleSubmit} className="relative flex items-end bg-slate-900 border border-slate-700/80 rounded-xl overflow-hidden focus-within:border-blue-500/50 focus-within:ring-1 focus-within:ring-blue-500/50 transition-all shadow-inner">
          <button type="button" className="absolute left-3 bottom-3 text-slate-500 hover:text-blue-400 transition-colors z-10">
            <Paperclip size={18} />
          </button>
