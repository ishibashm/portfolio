...
export const DynamicCanvas = ({ widgets }: DynamicCanvasProps) => {
  const { removeWidgetFromCanvas, updateWidgetSize, knowledgeSuggestions, dismissSuggestion, addWidgetToCanvas } = useOmniStore();

  const handleOpenKnowledge = (suggestionId: string) => {
    // 投資・企業分析に特化したジャーニーに合わせて、サジェストからTradingViewを開く
    addWidgetToCanvas({ type: 'tradingview', data: { symbol: 'TSE:6838', name: '多摩川HD' }, title: 'Charts & Financials', size: 'full' });
    ...
    <aside className="flex-1 bg-[#050810] overflow-y-auto p-4 md:p-6 lg:p-8 relative custom-scrollbar flex flex-col">
      <div className="flex justify-between items-center mb-6 px-2">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-bold tracking-widest text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded border border-emerald-500/20">LIVE</span>
          <span className="text-[10px] text-slate-400 font-mono bg-slate-900/80 px-3 py-1 rounded border border-slate-700/50 shadow-inner">4K OPTIMIZED</span>
        </div>
      </div>
      ...
      {knowledgeSuggestions && knowledgeSuggestions.length > 0 && (
        <div className="mb-8 px-2">
          {knowledgeSuggestions.map(suggestion => (
            <div key={suggestion.id} className="relative group bg-indigo-950/40 border border-indigo-500/30 rounded-xl p-5 shadow-lg shadow-indigo-500/10 overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-4 transition-all hover:bg-indigo-900/40 hover:border-indigo-400/50">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs font-bold text-indigo-400 tracking-wider bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-500/20 uppercase">Aha Moment</span>
                    <h3 className="text-base font-semibold text-slate-200">{suggestion.title}</h3>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-3 text-xs font-mono text-slate-500">
                    <span className="flex items-center gap-1 bg-slate-800/50 px-2 py-1 rounded border border-slate-700">
                      {suggestion.sourceType === 'note' && <svg className="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>}
                      {suggestion.sourceType === 'x' && <svg className="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4l11.733 16h4.267l-11.733-16z"/><path d="M4 20l6.768-6.768m2.46-2.46L20 4"/></svg>}
                      Source
                    </span>
                    <span className="text-indigo-400 border border-indigo-500/30 bg-indigo-500/10 px-2 py-0.5 rounded-full text-[10px] tracking-widest">{suggestion.relation}</span>
                    <span className="flex items-center gap-1 bg-slate-800/50 px-2 py-1 rounded border border-slate-700">
                      {suggestion.targetType === 'note' && <svg className="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>}
                      {suggestion.targetType === 'x' && <svg className="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4l11.733 16h4.267l-11.733-16z"/><path d="M4 20l6.768-6.768m2.46-2.46L20 4"/></svg>}
                      Target
                    </span>
                  </div>
                </div>
      ...
      {widgets.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center text-slate-500 opacity-50">
          ...
          <p className="font-bold text-slate-300 text-lg tracking-wide mb-2">No Active Widgets</p>
          <p className="text-sm">左側のサイドバーからワークスペースを選択するか、</p>
          <p className="text-sm">チャットでAIに指示を出してツールを展開してください。</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 auto-rows-min gap-4 lg:gap-6 pb-20">
          {widgets.map((widget, idx) => {
            ...
            let colSpan = 'col-span-1 md:col-span-1 xl:col-span-1 2xl:col-span-1';
            ...
            if (widgets.length === 1) {
              ...
              switch (widget.size) {
                case 'full':
                  ...
                  break;
                case 'wide':
                  colSpan = 'col-span-1 md:col-span-2 xl:col-span-2 2xl:col-span-3';
                  heightClass = 'h-[500px] 2xl:h-[600px]';
                  rowSpan = 'row-span-2';
                  break;
                case 'tall':
                  colSpan = 'col-span-1 md:col-span-1 xl:col-span-1 2xl:col-span-1';
                  heightClass = 'h-[650px] 2xl:h-[750px]';
                  rowSpan = 'row-span-2';
                  break;
                case 'half':
                  colSpan = 'col-span-1 md:col-span-1 xl:col-span-1 2xl:col-span-2';
                  heightClass = 'h-[400px]';
                  rowSpan = 'row-span-1';
                  break;
                case 'large':
                  colSpan = 'col-span-1 md:col-span-2 xl:col-span-2 2xl:col-span-2';
                  heightClass = 'h-[600px]';
                  rowSpan = 'row-span-2';
                  break;
                case 'normal':
            ...
            return (
              <div key={`${widget.type}-${idx}`} className={`${colSpan} ${rowSpan} bg-slate-900/70 border border-slate-700/60 rounded-xl flex flex-col shadow-2xl shadow-black/40 backdrop-blur-xl overflow-hidden transition-all duration-300 hover:border-slate-600/80 ${heightClass}`}>
                {widget.title && (
                ...
                <div className="flex-1 overflow-hidden h-full relative">
                  {widget.type === 'chart' && <TimesFMChart forecastData={widget.data.forecast || []} />}
                  {widget.type === 'draft_editor' && <SocialDraftEditor draftText={widget.data.content || ''} />}
                  ...
                  {widget.type === 'markdown_viewer' && <MarkdownViewerWidget />}
                  {widget.type === 'tradingview' && <TradingViewWidget symbol={widget.data.symbol || "NASDAQ:AAPL"} name={widget.data.name} reason={widget.data.reason} />}
                  ...
                  {widget.type === 'knowledge_editor' && <KnowledgeEditorWidget />}
