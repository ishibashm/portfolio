 ...
 * このサイトは珍しく**両方**を持っている。
 *
 *   購入  property_transactions   国交省の成約価格 247 万件（実際に成立した額）
 *   賃貸  rental_properties       自前の収集 99 万件（募集賃料）
 *
 * 件数は 2026-08-22 の実測。以前ここには 206 万 / 45 万と書いてあったが、
 * どちらも古かった。取り込みが進むので、**数字を書くときは日付を添える。**
 *
 ...
 * 大きい。
 *
 * **3-b. 分母は中古マンションだけ、分子は構造を選べない。**購入側は
 * 「中古マンション等」に絞ってある（種別によって ㎡ の意味が違うため。
 * 詳細は scripts/build_yield_stats.ts）。一方 rental_properties には
 * 構造の列が無いので、分子には木造アパートも RC マンションも混ざる。
 * **木造の㎡賃料は RC より安いので、利回りはやや低めに出る。**
 *
 * これらを画面に書かずに数字だけ出すと、**根拠のある数字に見えてしまう。**
 *
 * **4. 地図の白いところは「成約価格が無い」ではない。**実測（2026-08-22）で、
 * 賃貸が 5 件以上ある区画 3,289 のうち **98.9%（3,253）は成約側もそろう**。
 * 制約は賃貸側で、白いのは「賃貸を集めていない場所」。読む人は逆に
 * 受け取りやすいので、画面にそう書くこと。
 */
