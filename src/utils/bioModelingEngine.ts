...
export function calculateBioMetrics(params: BioModelParams): BioModelResult {
  ...
  let circadianMultiplier = 1.25 - 0.25 * Math.cos((solarTimeHours - 12) * Math.PI / 12);
  ...
  const pressurePenalty = pressureDrop < 0 ? Math.min(30, Math.abs(pressureDrop) * 3) : 0;
  ...
  const synergisticEnvironmentLoad = combinedEnvBase * (1 + (kpBasePenalty > 0 && (elevationPenalty > 0 || pressurePenalty > 0) ? 0.3 : 0));
