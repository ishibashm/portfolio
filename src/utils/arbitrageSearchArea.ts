...
/**
 * その表示範囲を、押さずに走査してよいか（**面積**の規則）。
 *
 * 止めるのは**県も半径も未指定で、かつ範囲が広すぎる**ときだけ。県か
 * 半径があれば、ここでは止めない。
 *
 * 「県や半径が選ばれていれば母数がそちらで絞られる」と書いていたが、
 * **半径については誤りだった**（#1159 の前提。2026-09-11 に判明）。
 * 都心の半径 50km は表の 4 分の 3（33 万行超）に当たるうえ、#1183 まで
 * API は半径があると表示範囲を捨てていた。半径は面積の代わりにならない。
 * 行数で見るのは下の `shouldPauseForRows`（#1178）で、こちらは面積の
 * 規則としてだけ残す。
 */
 ...
 *     305,200 行以上       → 25〜41 秒
 *
 * **上の表の秒数は「同じ走査が 2 本同時に走っていた」ときのもの**
 * （2026-09-11 に判明。backlog 19 節「続報 2」）。遅い回は全部が 1〜2 秒
 * 差の対で、画面が 2 本投げていた（#1184 で 1 本ずつに）。1 本ずつなら
 * db-explain で 52〜56 万行が 4.5〜4.9 秒。db-explain の 880ms（126,800
 * 行）が「本番の 20 分の 1」に見えたのは、本番の側が 2 本重ねていた
 * ため。表は #1183・#1184 の後に測り直したら更新する。
 *
