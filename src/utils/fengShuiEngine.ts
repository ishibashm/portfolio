 ...
 * ## 暦計算はここに置かない
 *
 * この判定は表引きだけで、暦の計算を使わない。生年月日 → 節年の変換
 * （`honmeiYearFor`）は `utils/honmeiYear` に分けてある。あちらは
 * lunar-javascript（約 300KB）を値として引くので、同じファイルに置くと
 * 早見表しか出さない画面（`/houi`）のバンドルにまで暦が乗る。
 */
