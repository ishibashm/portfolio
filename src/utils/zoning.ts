 ...
 *
 * **z10 以下は上流が受け付けない。**probe（run 33946832674、2026-09-05）で
 * z5〜10 を投げたところ、全部が
 *
 *   HTTP 400 {"message":"不正なズーム値（z）が指定されたため、検索できませんでした。"}
 *
 * だった（404 ではなく 400。ズームの非対応は本文で分かる）。z11 と z12 は
 * 前回と同じ件数・バイト数。したがって**全国の俯瞰（z5〜9）に用途地域を
 * 出すなら、タイル API からは組めない**（z5 の 1 枚 = z11 の 4,096 枚）。
 * 別の入手経路（国土数値情報 A29 の一括ファイルなど）を先に確かめる。
 */
