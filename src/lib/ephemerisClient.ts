...
export interface EphemerisData {
  source: string;
  planetaryPositions: Record<string, { longitude: number, latitude: number, constellation: string }>;
}
...
export class EphemerisClient {
  public getPositions(date: Date = new Date()): EphemerisData {
    ...
    const positions: Record<string, { longitude: number, latitude: number, constellation: string }> = {};
