  ...
  for (const raw of lines) {
    ...
    if (line === "") {
      /*
        ...
        **短い導入文はここで確定させない。**「理由は 3 つあります。」
        「まず、占術ではなく地図の話です。」のように、見出しの直後が
        次を指すだけの 1 文で、中身は次の段落にある書き方が多い。
        1 段落で切ると 30 字の下限を割って**まるごと捨てられる。**

        実測（2026-09-15）では 31 本中 FAQPage が出ていたのは **9 本**で、
        **13 本が「あと 1 組」で止まっていた。**その多くがこれ。

        繋いでよいのは**見出しの直下に続いている地の文**だけ。表・箇条書き・
        引用・コードが来たら下の枝で打ち切るので、そこは変わらない。
        頁に出ている文言をそのまま使うという約束（この節の冒頭）は保つ
        （段落を空白 1 つで繋ぐだけなので、頁の文字列の中に現れる）。

        **箇条書きまで繋ぐのは見送った。**「理由は 3 つあります。」の
        直後に並ぶ項目は確かに答えの続きなのだが、1 行に均すと
        「登録制度がない 分派が続いている」と 1 文に読めてしまう。区切りに
        `／` を入れると読めるようになる代わりに、**頁に無い文字を答えに
        混ぜることになり**、「答えは頁に実際に出ている文言でなければ
        ならない」を破る（この検査は `articleFaq.test.ts` にあって、実際に
        止められた）。見た目のために捏造しない。
      */
      if (
        buffer.length > 0 &&
        plainText(buffer.join(" ")).length >= MIN_ANSWER
      ) {
        flush();
      }
      continue;
