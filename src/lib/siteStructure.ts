...
export const ROUTE_GROUPS: {
  ...
  /*
    **「いつ」を先に出す**（利用者の指示、2026-09-06）。引越しは時期の
    ほうが先に決まっていることが多く、方位はその中から選ぶ。名前の順が
    「どこへ・いつ・いくら」なのは問いの言い回しで、開く順とは別。

    ここが**唯一の定義元**で、サイドバーとホームの両方がこの順で並ぶ。
    片方だけ変えると定義が 2 つに割れるので、順序はここでだけ決める。
  */
  {
    key: "timing",
    label: "いつ — 時期を選ぶ",
 ...
 *
 * **この配列の順序は ROUTE_GROUPS とは別。**画面に出る群の順序は
 * ROUTE_GROUPS が決め、ここは群ごとにまとめてあるだけ。先頭
 * （CORE_ROUTES[0]）は**代表の入口**としてホームの主ボタンと
 * llms.txt の案内文が名指しで使うので、並べ替えると出す先が黙って
 * 変わる。動かすときはその 2 か所を確かめること。
 */
export const CORE_ROUTES: CoreRoute[] = [
  // ── どこへ（方位で選ぶ）─────────────────────────────
  {
    ...
    label: "方位で街を探す",
    /* 2026-09-20 に「物件を方位で探す」から改名。掲載の取り込みを止めた
       （backlog 29 節）ので部屋は並べず、方位ごとの街を並べる。URL は据え置き。 */
    summary:
      "今住んでいる場所から見た八方位の吉凶と、それぞれの方位にある市区町村の家賃の水準・空き家率（e-Stat）を並べる。物件の掲載は出さない。",
    group: "direction",
  },
  {
  ...
  // ── いつ（時期を選ぶ）───────────────────────────────
  /*
    日取り（/calendar）を先に置く（利用者の判断、2026-09-26「引越しの
    日取りを選ぶの方がサイドバーだと上の方になると思う」）。まず候補の日を
    出し、全期間の分析（/relocation/timing）はその次に詰める順
  */
  {
    href: "/calendar",
    label: "引越しの日取りを選ぶ",
  ...
  // ── いくら（相場を知る）─────────────────────────────
  {
    ...
    label: "家賃相場を分析する",
    summary:
      "全国の賃貸掲載を、ヘドニック回帰・分布分析・生存分析で集計した結果。条件相応の家賃、割安・割高の分布、掲載の消化速度、市区町村ごとの価格のばらつきを見る。掲載の取り込みは規約に従って止めたので、集計はその時点のもの。",
    group: "market",
  },
  {
    ...
    label: "購入の相場を分析する",
    summary:
      "国交省の成約価格をもとに、㎡単価・土地代と建物代の比率・築年数・構造・都道府県別の相場を毎週集計する。地価公示との対比も出す。売り出し価格ではなく実際に取引が成立した額を扱う。",
    group: "market",
  },
  {
    /*
...
/**
 * 道具の名前。**`CORE_ROUTES` が正で、写さずにここから引く。**
 *
 * 2026-09-14 に実測したところ、「物件を方位で探す」という文字列が
 * **17 ファイル**に写されていた（ナビ・ホーム・ガイド・記事・
 * 県ページ・市区町村ページ・MCP の説明文など。行数では 25）。`CORE_ROUTES` が正の値を持って
 * いるのに、参照しているのはナビだけだった。
 *
 * 賃貸の巡回を止めたので（backlog 29 節）、掲載は 2026 年 10 月中旬に
 * 0 件になる。**部屋を並べなくなった時点でこの名前は実態とずれる**ので、
 * 名前を変える必要がある（利用者の判断 A、2026-09-14。URL は据え置き）。
 * 手で直すと必ず取りこぼすため、先に引く形へ寄せる。
 *
 * 知らない `href` は `null`。**当てずっぽうの名前を作らない**（綴り違いで
 * 黙って別の名前が出ると、ナビと本文で食い違う）。
 */
export function coreRouteLabel(href: string): string | null {
  return CORE_ROUTES.find((r) => r.href === href)?.label ?? null;
}
