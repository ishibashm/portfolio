...
function parseFrontmatter(source: string, slug: string): BlogPost {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(publishedAt)) {
    throw new Error(`ブログ記事 ${slug} の publishedAt が YYYY-MM-DD ではありません。`);
  }
  if (updatedAt && !/^\d{4}-\d{2}-\d{2}$/.test(updatedAt)) {
    throw new Error(`ブログ記事 ${slug} の updatedAt が YYYY-MM-DD ではありません。`);
  }
  const draftValue = metadata.get("draft")!;
  if (draftValue !== "true" && draftValue !== "false") {
    throw new Error(`ブログ記事 ${slug} の draft は true か false にしてください。`);
  }
