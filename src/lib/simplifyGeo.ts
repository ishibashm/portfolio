 ...
 * ## なぜ要るか
 *
 * 用途地域の下限ズームは重さの実測で決まっている（`utils/zoning` の
 * `ZONING_MIN_ZOOM` に表）。**広い縮尺ほど頂点は要らない**——z12 で
 * 1 ピクセル未満しか動かない折れ点は、描いても見えない。間引けば形を
 * ほぼ保ったまま小さくできる。どこまで小さくなるかは推測せず
 * `scripts/probe_zoning_simplify.ts` で実物を測り、その結果で下限を
 * z14 → z13 に下げた（z12 は間引いても 1 画面 6.5MB で見送り）。
 * `/api/zoning` が返す前にこの間引きを通す。
 *
 * ## 判定には使わない
 *
