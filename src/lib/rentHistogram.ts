...
/**
 * 升番号を出す SQL 式。**定義はここ 1 か所。**
 *
 * 呼ぶ側が 2 つある（API の口と、実行計画を測るスクリプト）。式を
 * 写すと、片方だけ升の幅を変えたときに「測った計画」と「本番の
 * クエリ」が別物になる。それでは測る意味が無い。
 *
 * 引数は埋め込みの定数にしてある。プレースホルダで渡すと width_bucket
 * の多重定義（numeric 版と double precision 版）の解決が型推論に
 * 依存し、環境によって選ばれる関数が変わりうる。
 */
export const RENT_BUCKET_SQL = `width_bucket(rent + coalesce(management_fee, 0), 0, ${OVERFLOW_FLOOR_YEN}, ${BUCKET_COUNT - 1})`;
