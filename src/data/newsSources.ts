 ...
 * 不動産・建築の情報源の台帳。
 *
 * **2026-09-21 に、この台帳を読んでいた画面と取得の仕組みを全部閉じた**
 * （/news・市区町村と県ページの見出し・/api/news/local・lib/fetchNews ほか）。
 * 商用・業界メディアの RSS 利用規約をこちらで確認できていないため
 * （開発環境から規約ページを読めない。CLAUDE.md 3 節）。**台帳だけ残す。**
 * どの配信元をどう調べたか——生きている URL、落ちたフィード、403 で断られて
 * リンクに降ろした媒体——の記録で、規約を人が読んで確かめたらここから戻せる。
 * 以下の「載せ方の決まり」は、戻すときの前提としてそのまま残してある。
 *
 * ## 載せ方の決まり
 *
...
export interface FeedSource {
  ...
  section?: string;
  /**
   * 地域の配信であれば、その都道府県名（「新潟県」のように接尾辞ごと）。
   *
   * 主要メディアの県ごとの配信を載せるための属性（2026-09-12。利用者の
   * 依頼「地域のニュースを主要新聞・主要メディアから、選択した各地域で
   * 表示したい」）。付いたフィードの見出しは、**地名の一致に頼らず**
   * その県の頁と /news の県の選択欄に出る（`lib/localNews`）。全国の
   * 配信には付けない。付けると、その県と無関係の見出しまで県の頁に出る。
   *
   * 出どころを足す前に、その配信の利用条件を人が読む（CLAUDE.md 3 節
   * 「取得してよいかは、規約で決まる」）。
   */
  pref?: string;
}
