...
export interface WidgetConfig {
  type: 'chart' | 'draft_editor' | 'youtube_extractor' | 'youtube_chat' | 'xtrends' | 'markdown_viewer' | 'data_analyzer' | 'tradingview' | 'pine_editor' | 'jsonl_splitter' | 'omni_pipeline' | 'knowledge_editor' | 'ir_analysis';
  data: any;
  title?: string;
...
const workspaceDefaults: Record<string, WidgetConfig[]> = {
  'ws-1': [
    { type: 'data_analyzer', data: {}, title: 'JSONL形態素解析・キーワード抽出 (AI分析)', size: 'full' },
    { type: 'tradingview', data: {}, title: 'Charts & Financials', size: 'full' },
    { type: 'ir_analysis', data: {}, title: 'IR & EDINET Analysis', size: 'large' },
  ],
  'ws-2': [],
  'default': [
    { type: 'data_analyzer', data: {}, title: 'X Information & Financial Analysis', size: 'full' },
    { type: 'tradingview', data: {}, title: 'Charts & Financials', size: 'full' },
    { type: 'ir_analysis', data: {}, title: 'IR & EDINET Analysis', size: 'large' },
  ]
};
...
export const useOmniStore = create<OmniTerminalStore>()(
  persist(
    (set) => ({
      ...
      globalWatchlist: ['NASDAQ:AAPL', 'NASDAQ:TSLA', 'NASDAQ:NVDA', 'CRYPTO:BTCUSD'],
      knowledgeSuggestions: [
        {
          id: 'sug-1',
          title: '企業名の特定とチャート連携',
          description: 'Xの情報収集から「多摩川くん」という隠語を検出し、企業名「多摩川ホールディングス (6838)」を特定しました。チャートと業績を確認しますか？',
          sourceType: 'x',
          targetType: 'note',
          relation: 'IDENTIFIED',
      ...
      addWidgetToCanvas: (widget) => set((state) => {
        let newWidgets = [...state.canvasWidgets];
        
        if (widget.type === 'tradingview') {
          const existingIndex = newWidgets.findIndex(w => w.type === 'tradingview');
          if (existingIndex >= 0) {
            newWidgets[existingIndex] = { ...newWidgets[existingIndex], ...widget };
            return { canvasWidgets: newWidgets };
          }
        }
        if (widget.type === 'pine_editor' || widget.type === 'data_analyzer' || widget.type === 'omni_pipeline' || widget.type === 'knowledge_editor' || widget.type === 'ir_analysis') {
          const existingIndex = newWidgets.findIndex(w => w.type === widget.type);
          if (existingIndex >= 0) {
      ...
      removeWidgetFromCanvas: (index) => set((state) => ({
        canvasWidgets: state.canvasWidgets.filter((_, i) => i !== index)
      })),

      updateWidgetSize: (index, size) => set((state) => {
      ...
      addToWatchlist: (tickers: string[]) => set((state) => {
      ...
      dismissSuggestion: (id) => set((state) => ({
        knowledgeSuggestions: state.knowledgeSuggestions.filter(s => s.id !== id)
      }))
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state && state.activeWorkspaceId && state.workspaces) {
          state.canvasWidgets = state.workspaces[state.activeWorkspaceId] || workspaceDefaults[state.activeWorkspaceId] || [];
        }
      },
