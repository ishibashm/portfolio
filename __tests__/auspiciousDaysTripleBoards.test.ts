...
describe("三盤吉は三つとも吉であること", () => {
  ...
  it("旧条件で三盤吉だった日の振り分け（2026〜2027 の 2 年・全方位）", () => {
    ...
    /* 月交点を判定から外したぶん（2026-09-19）、旧 S の母数が 309 → 338 に
       増え、A・B への振り分けも増えた。**割れ方の性質は変わっていない**
       （下の等式）。

       土用の日を日本時間の日の終わりで切り直して（#1493）338 → 337。
       土用殺の掛かる日が 1 日ずつ前へ動いたので、2 年 × 8 方位のうち
       1 通りが旧 S から外れ、その内訳ぶん B が 217 → 216 になった。 */
  ...
  it("段階 A（吉2盤・凶なし）が実際に出るようになる", () => {
    ...
    /* 月交点を外して 101 → 107（2026-09-19）。0 件でないことが芯。 */
    expect(a).toBe(107);
  });
