...
describe("30 日予報の評価時刻", () => {
  ...
  it("旧実装は節入りが日中に来る日で地図と食い違う（この修正の対象）", () => {
    ...
    // 拾い直した実例。どれも節入りが日中に来る日。
    //
    // **21 時台の例をここから外した（2026-09-07）。**
    // 「太陽時に直すと翌日になる時間帯」として 1/15 21:30 を入れて
    // いたが、ずれの本当の原因は太陽時ではなく、独自モデルの日盤が
    // ユリウス日（世界時の正午で繰り上がる）で切られていたことだった。
    // その元を断ったので、いまは旧実装でも地図と一致する。
    // 詳しくは __tests__/physicalDayStarJstBoundary.test.ts。
    //
    // 残り 6 件は節入りの時刻そのものが理由なので、旧実装ではいまも
    // 落ちる（この検査が空回りしていないことの担保）。
