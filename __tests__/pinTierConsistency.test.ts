...
describe("単盤と三盤は実際にずれる（食い違いの再現）", () => {
  ...
  it("同じ方位でも日によって段階が変わる（月のセル＝その月の最良、と読み違えないこと）", () => {
    ...
    // 8/10 の北西は X（日盤が歳破/月破）だが 8/12 の北西は A（年盤と
    // 日盤が吉・凶なし）。マトリクスの 08 列がその月の最良を出していても、
    // 選択日（8/10）の地図が赤いのは矛盾ではない。
    //
    // 8/12 は #566 まで S だった。三盤吉の条件が「最終が吉＋凶なし」で、
    // 年盤 1 枚が吉なら S になっていたため。いまは月盤が SAFE なので
    // 吉 2 枚の A になる。この test が見たいのは「日で段階が変わる」
    // ことなので、段階の文字が変わっても趣旨は同じ。
    expect(gradeVerdict(d10.NW)).toBe("X");
    expect(gradeVerdict(d12.NW)).toBe("A");
  });
});
