...
describe("立春をまたぐ盤", () => {
  ...
  /*
    先の年も分単位で固定する（2026-09-08 に +2 年、2026-09-13 に +3 年）。
    ...
    **2029 は 2 月 3 日。**立春が 2 月 4 日でない年があり、「2 月 4 日で
    切り替える」と決め打ちした実装なら**この行だけが落ちる。**日付を
    またぐ年を 1 つ入れておくために、下の 2029〜2031 は別に書いてある
    （2026-09-13 の精度確認。2029 が 2 月 3 日であることは外部の資料でも
    確かめた）。
  */
  it.each([
  ...
  /*
    日付をまたぐ年を含めて、さらに 3 年ぶん固定する（2026-09-13）。
    **2029 の立春は 2 月 3 日 22:21 JST。**月日を決め打ちした実装に
    なっていないかを、ここで毎回確かめる。
  */
  it.each([
    ["2029-02-03", "22:20", "22:22", 8, 7, 9, 8],
    ["2030-02-04", "04:08", "04:10", 7, 6, 6, 5],
    ["2031-02-04", "09:58", "10:00", 6, 5, 3, 2],
  ] as const)(
    "%s %s→%s の間で年盤 %i→%i・月盤 %i→%i に替わる",
    (day, before, after, y0, y1, m0, m1) => {
      expect(getClassicalYearStar(jst(`${day}T${before}:00+09:00`))).toBe(y0);
      expect(getClassicalYearStar(jst(`${day}T${after}:00+09:00`))).toBe(y1);
      expect(getClassicalMonthStar(jst(`${day}T${before}:00+09:00`))).toBe(m0);
      expect(getClassicalMonthStar(jst(`${day}T${after}:00+09:00`))).toBe(m1);
    },
  );
