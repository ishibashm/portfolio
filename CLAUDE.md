...
**分かれ目は「道具か記事か」ではなく「長い本文があるか」。**索引の頁は記事の
下にあっても本文が無いので、記事と同じ幅で止めない（#238・#239 で直した）。

| 幅 | どの頁 | 理由 |
|---|---|---|
| `max-w-[2560px]` | 地図と一覧を左右に並べる（`arbitrage`・`wealth`） | 余った幅がそのまま地図の描画面積になる |
| `max-w-[1700px]` | それ以外の道具（ホーム・`simulator`・`timing`・`history`・`market`・`calendar`）と、**札やリンクを並べる索引**（`guide`・`houi`・`houi/area`） | 表・升目・札が広いほど一度に見渡せる |
| 680〜860px | **本文が長い記事**（`guide/[slug]`・`houi/[year]/[star]`・`houi/area/[code]`・`about`・`contact`） | **広げない。**日本語は文字ごとに折り返せるので、1700px の行は行末から次の行頭へ目を戻せない |
