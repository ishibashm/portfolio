...
- **判定ロジック**（しきい値・条件式・数値）を変える。3 節のとおり
- 判定の**見え方**が変わる（方位の割り当て、段階の境目、色の意味）
- ファイル・コンポーネントを丸ごと消す
- **新しい機能・ページ・API・DB のテーブルを足す**
- 手で適用が要る変更（`prisma/sql/` の DDL など）。出すのはよいが、
  **適用の手順を必ず本文に書く**
- 記事の文言。流派の判断が入る
- 個人情報・プライバシーの扱い（生年月日の既定値、ログに残す項目）
- `docs/improvement-backlog.md` で「判断待ち」に入れてあるもの
- 本番へ出るもの、外部サービスに課金が発生するもの
...
- 「進めてよいもの」のつもりで始めた作業が、途中で判定に触り始めた
