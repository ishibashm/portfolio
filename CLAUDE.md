...
（**331 頁**。2026-09-07 実測。真の行き止まりがある頁は 171 で、
そのうち**未着手は 5**＝鹿児島の薩摩川内市・出水市、長崎の平戸市、
岩手の陸前高田市、和歌山の新宮市）。
...
**巡回で代表点が動くと顔ぶれごと変わる。**県ページ（`prefEditorial`）は
47 県ぶん全部そろっている。
...
- 市区町村ページの空 … **4 通りある。**`emptyDirections` が
  ...
  分ける（#835〜#837、#968）。空の 711 方位の実測は次のとおり
  （2026-09-07。**総数は巡回で代表点が動くたびに変わる**ので、
  言う前に必ず測り直す。3 日前は 712 方位で 113 / 373 / 6 / 220 だった）

  | 種別                             | 件数 | 判定                        |
  | -------------------------------- | ---- | --------------------------- |
  | 遠いだけ                         | 113  | `hasBeyondRange: true`      |
  | 掲載漏れ（街はあるが掲載が無い） | 365  | `hasAnyMunicipality: true`  |
  | 近すぎて外れているだけ           | 6    | `hasNearMunicipality: true` |
  | 真の行き止まり                   | 227  | どれも false                |
  - **「海や山だから行き止まり」と書かない。**掲載を集計できた市区町村
    は全国 1,917 のうち 1,141 しか無く（2026-09-07 実測）、掲載の側
    だけを見ると実在する
