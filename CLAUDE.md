...
- 市区町村ページの空 … **4 通りある。**`emptyDirections` が
  ...
  分ける（#835〜#837、#968）。空の 712 方位の実測は次のとおり
  （2026-09-04。**総数は巡回で代表点が動くたびに変わる**ので、
  言う前に必ず測り直す）

  | 種別                             | 件数 | 判定                        |
  | -------------------------------- | ---- | --------------------------- |
  | 遠いだけ                         | 113  | `hasBeyondRange: true`      |
  | 掲載漏れ（街はあるが掲載が無い） | 373  | `hasAnyMunicipality: true`  |
  | 近すぎて外れているだけ           | 6    | `hasNearMunicipality: true` |
  | 真の行き止まり                   | 220  | どれも false                |
  - **「海や山だから行き止まり」と書かない。**掲載を集計できた市区町村
    は全国 1,917 のうち 1,119 しか無く、掲載の側だけを見ると実在する
