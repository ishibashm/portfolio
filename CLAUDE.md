...
（#750〜#828 で **194 頁**。2026-08-31 時点）。県ページ（`prefEditorial`）は
47 県ぶん全部そろっている。

**選ぶ順序は 3 回変えた。**最初は掲載の多い順、次に書いた頁の少ない県から、
今は**空の方位がある頁を優先**している。「その方位に街が無い」がこの頁で
いちばん効く情報なので、そこが書ける頁から埋めるほうが順序として正しい。
同じ市の別の区は**選ぶ前に候補から外す**（構造が似て下の類似検査に掛かる）。
...
- 市区町村ページの空 … 5〜150km に街が無い。海や山の**行き止まり**と、
  「150km より先にはある」を `hasBeyondRange` で分ける（#790 の訂正。
  709 の空のうち 114 は行き止まりではなかった）
  - **「海だから行き止まり」と書きたくなるが、そうとは限らない。**
    新潟市西区の北（#823）と南相馬市の北東（#826）で 2 回続けて
    間違え、2 回とも `areaEditorialDeadEnds` が入口で止めた。
    **書く前に `emptyDirections` の `hasBeyondRange` を 1 つずつ見る**
