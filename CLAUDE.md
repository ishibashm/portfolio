...
| `max-w-[1700px]` | それ以外の道具（ホーム・`simulator`・`timing`・`history`・`market`・`calendar`） | 表と升目が広いほど一度に見渡せる |
| 680〜860px | 記事（`houi/*`・`guide/*`・`about`・`contact`） | **広げない。**日本語は文字ごとに折り返せるので、1700px の行は行末から次の行頭へ目を戻せない |
