...
- 市区町村ページの空 … **3 通りある。**`emptyDirections` が
  `hasBeyondRange` と `hasAnyMunicipality` で分ける（#835〜#837）。
  空の 709 方位の実測は次のとおり

  | 種別                             | 件数 | 判定                       |
  | -------------------------------- | ---- | -------------------------- |
  | 遠いだけ                         | 114  | `hasBeyondRange: true`     |
  | 掲載漏れ（街はあるが掲載が無い） | 368  | `hasAnyMunicipality: true` |
  | 真の行き止まり                   | 227  | どちらも false             |
