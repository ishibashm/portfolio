...
async function runDiagnostics() {
  ...
  check(hasRuleLayer, 15, 'ルール層 (CLAUDE.md 等) が定義されています。', 'ルール層が見つかりません。エージェントの行動が不安定になる可能性があります。');

  if (hasClaudeMd) {
    const content = await readFileSafe('CLAUDE.md');
    const hasConstraints = content && (content.includes('Constraint') || content.includes('制約') || content.includes('Never'));
    check(hasConstraints, 5, 'CLAUDE.md に明確な制約（Constraints）が記述されています。', 'CLAUDE.md に明確な制約が見当たりません。安全な運用のため追記を推奨します。');
  } else {
  ...
  check(hasMemoryMd, 15, 'コンテキスト層 (MEMORY.md) が定義されています。状態の引継ぎが可能です。', 'MEMORY.md が見つかりません。セッション間で文脈が失われる可能性があります。');
  ...
  check(hasLinter, 10, 'Linter (ESLint) が設定されています。フィードバックループが早期化されます。', 'Linter が見つかりません。エージェントが静的エラーに気付きにくくなります。');
  check(hasFormatter, 5, 'Formatter (Prettier) が設定されています。', 'Formatter が見つかりません。コードスタイルのブレが発生する可能性があります。');
  check(hasTesting, 15, 'テストフレームワークが設定されています。品質検証フックとして機能します。', 'テストフレームワークが見つかりません。「完了しました」の客観的検証が難しくなります。');
  check(hasTypeScript, 10, 'TypeScriptが導入されています。型による強力なガードレールが機能します。', 'TypeScriptが見つかりません。型の恩恵を受けられません。');

  // 4. tsconfig.json の厳格さ
  const tsconfigRaw = await readFileSafe('tsconfig.json');
  if (tsconfigRaw) {
    const hasStrict = tsconfigRaw.includes('"strict": true');
    check(hasStrict, 10, 'tsconfig で strict モードが有効です。', 'tsconfig で strict モードが無効、または明記されていません。');
  } else {
  ...
  check(hasGithubActions, 10, 'CI/CD (GitHub Actions等) の設定があります。継続的インテグレーションが保護されています。', 'CI/CDの設定が見つかりません。破壊的変更が本番に流れるリスクがあります。');
