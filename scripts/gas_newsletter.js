...
/*
  Google Apps Script が**名前で呼ぶ入口**。このファイルの中から呼ばれる
  ことは無いので、そのままだと eslint が「未使用」と言う。消すと動いて
  いる Apps Script が壊れる（CLAUDE.md 4 節の表）。

  ESLint 標準の `exported` 宣言で「外から使われる」と書く。黙らせる
  （eslint-disable）のではなく事実を書く形で、本当に未使用の関数が
  増えたときはそれだけが警告に残る。
*/
/* exported doPost, doGet, sendDailyTechDigest */
...
function doPost(e) {
  try {
    ...
    for (let i = 1; i < values.length; i++) {
      ...
      if (
        registeredEmail &&
        registeredEmail.toString().toLowerCase() === email.toLowerCase()
      ) {
        return createJsonResponse({
          status: "duplicate",
          code: 409,
...
function doGet() {
  try {
    ...
    return ContentService.createTextOutput(errorJson).setMimeType(
      ContentService.MimeType.JSON,
    );
  }
}
...
function sendDailyTechDigest() {
  try {
    ...
    console.log(
      `${emails.length} 件のアドレスにまとめメールを正常に送信しました。`,
    );
  } catch (error) {
    console.error(
      "sendDailyTechDigest実行中に致命的なエラーが発生しました: " +
        error.toString(),
    );
  }
}
  ...
  FEED_CONFIGS.forEach(function (config) {
    try {
      ...
      if (response.getResponseCode() === 200) {
        ...
        console.warn(
          `${config.name} フィードの取得に失敗しました。HTTPステータス: ${response.getResponseCode()}`,
        );
        result[config.name] = [];
      }
    } catch (e) {
      console.error(
        `${config.name} のフェッチおよび解析中に例外が発生しました: ` +
          e.toString(),
      );
      result[config.name] = [];
    }
