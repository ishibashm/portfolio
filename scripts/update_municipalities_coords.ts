  ...
  try {
    for (const m of municipalities) {
      ...
      if (coords) {
        ...
        console.warn(
          `No coordinates found for areaCode: ${code} (${m.areaName})`,
        );
      }
    }
    console.log(
      `Matched and updated coordinates for ${matched} out of ${municipalities.length} municipalities in the database.`,
    );
  } catch (error) {
    console.error(error);
  } finally {
