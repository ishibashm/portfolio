...
import { SwissEphemerisEngine, CelestialBody } from '../src/utils/swissEphemerisEngine';
...
function triggerAgentEvolution(triggerType: string, details: string, value: any) {
  writeLog(`🔥 Triggering Agent Evolution. Reason: [${triggerType}] ${details}`);
  ...
  child.on('close', async (code) => {
    if (code !== 0) {
      writeLog(`[Error] Agent runner failed with code ${code}. Stderr: ${stderr}`);
      try {
    ...
    try {
      const match = stdout.match(/---AGENT_RESULT_START---([\s\S]*?)---AGENT_RESULT_END---/);
      if (!match) {
        throw new Error(`Failed to find structured JSON in runner output. Raw stdout: ${stdout}`);
      }
        ...
        for (const call of agentResponse.toolCalls) {
          if (call.name === 'set_color_theme') {
            try {
              ...
              themeApplied = true;
              writeLog(`🎨 [Theme Applied] New self-evolved theme written to DB.`);
            } catch (e: any) {
...
async function checkEnvironment(state: DaemonState): Promise<DaemonState> {
  ...
  try {
    ...
    writeLog(`[Oura Biometrics] Current User ANS Load: ${ansLoad}% (Readiness: ${score})`);
  ...
  try {
    ...
    for (const [name, body] of Object.entries(planetsToCheck)) {
      ...
      writeLog(`[Ephemeris] ${name} is Retrograde: ${isRetrograde} (Previous: ${previousState})`);
