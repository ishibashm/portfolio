...
async function main() {
    ...
    const files = fs.readdirSync(LOCAL_DIR).filter(f => f.endsWith('.png') || f.endsWith('.jpg'));
    ...
    for (const file of files) {
        ...
        if (updateResult.count > 0) {
            ...
            console.log(`-> Warning: No matching DB record found for ${file}, attempting backup match...`);
