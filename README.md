...
ナビとホームは `ROUTE_GROUPS` の 3 群（どこへ / いつ / いくら）で並ぶ。

| 群     | URL                     | 役割                                                 |
| ------ | ----------------------- | ---------------------------------------------------- |
| —      | `/`                     | ホーム。方位盤とヒートマップ、生年月日・現在地の設定 |
| どこへ | `/relocation/arbitrage` | 物件を方位で探す                                     |
| どこへ | `/relocation/simulator` | 引越し先を試算する                                   |
| どこへ | `/relocation/wealth`    | 移住先の地域を比べる                                 |
| どこへ | `/houi`                 | 本命星と吉方位を調べる（年 × 星 × 月の静的記事つき） |
| いつ   | `/relocation/timing`    | 引越し時期を分析する                                 |
| いつ   | `/calendar`             | 引越しの日取りを選ぶ                                 |
| いくら | `/relocation/market`    | 家賃相場を分析する                                   |
| いくら | `/relocation/purchase`  | 購入の相場を分析する                                 |
| いくら | `/relocation/appraisal` | 検討中の物件を査定する                               |

中核の 9 画面のほかに、ナビへ出すが `CORE_ROUTES` に入れない頁がある。

| URL      | 役割                                               |
| -------- | -------------------------------------------------- |
| `/news`  | 不動産・建築の新着見出し（配信元の RSS を 6 時間） |
| `/guide` | 使い方と九星気学の説明                             |
...
- **判定は必ず真北で行う。**磁北は「方位磁針で測るとずれる」注意としてのみ
  ...
  0（補正なし）
- **判定に使う時刻は「その日の日本時間の正午」。**`utils/boardInstant.forecastAnchorMs`
  が唯一の基準。`setHours` / `getFullYear` / `getDay` は実行環境のタイムゾーンで
  動くので、日取りの経路では使わない（本番は UTC、ブラウザは JST）
- **「三盤吉」は年・月・日が三つとも吉。**移転の最終判定は年盤と月盤しか見ないので、
  「最終が吉」だけを条件にすると 1 枚しか吉でない日まで数えてしまう
- **状態の日本語は `src/lib/directionLabels.ts` だけが持つ。**画面ごとに手元の表を
  作らない（`name` / `badge` / `short` / `detailed` の 4 形。呼び名の系統は揃える）
- 天中殺は解釈が流派で割れるため、扱いを 5 通り（厳格〜使わない）から選べる
- 西洋占星術の要素は判定に混ぜない（表示は参考情報のみ）
- **利用者の投稿欄は記事（`/blog/{slug}`）にだけ出す。**道具の画面や方位・暦の
  頁には出さない
